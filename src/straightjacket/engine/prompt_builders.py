...
def creativity_seed(n: int = 3) -> str:
...
def _npc_block(game: GameState, target_id: str | None,
    ...
    memories = retrieve_memories(target, context_text=context_text,
                                max_count=5, current_scene=game.narrative.scene_count)
    ...
    if observations:
        obs_text = " | ".join(
            f'{m.event}({m.emotional_weight})' for m in observations
        )
...
def _activated_npcs_block(activated: list[NpcData], target_id: str | None,
    ...
    for npc in activated:
        ...
        if target_id and (npc.id == target_id or
                          npc.name.lower() == str(target_id).lower()):
            continue
        memories = retrieve_memories(npc, context_text=context_text,
                                     max_count=2, current_scene=game.narrative.scene_count)
        mem_hint = ""
        if memories:
            reflections = [m for m in memories if m.type == "reflection"]
            if reflections:
...
def _known_npcs_string(mentioned: list[NpcData], game: GameState,
                       exclude_ids: set | None = None) -> str:
    ...
    player_loc = (game.world.current_location or "").lower()
    parts = []
...
def _pacing_block(game: GameState, chaos_interrupt: str | None = None,
...
def build_dialog_prompt(game: GameState, brain: BrainResult, player_words: str = "",
    ...
    if dg and dg.narrator_guidance:
...
def build_action_prompt(game: GameState, brain: BrainResult, roll: RollResult,
                        consequences: list[str], clock_events: list, npc_agency: list[str],
                        player_words: str = "",
                        chaos_interrupt: str | None = None,
                        activated_npcs: list[NpcData] | None = None,
                        mentioned_npcs: list[NpcData] | None = None,
...
def build_epilogue_prompt(game: GameState) -> str:
    ...
    endings_text = ", ".join(
        f'{e.type}: {e.description}' for e in endings
...
def build_new_chapter_prompt(game: GameState) -> str:
    ...
    if game.campaign.campaign_history:
        ...
        if evolutions:
            evo_lines = "\n".join(
                f'  {_xe(e.name)}: {_xe(e.projection)}'
                for e in evolutions if e.name and e.projection
            )
            evolutions_block = (
