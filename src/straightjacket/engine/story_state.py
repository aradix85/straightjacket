...
def get_current_act(game: GameState) -> CurrentAct:
    ...
    if bp.story_complete and game.campaign.epilogue_dismissed:
        last = bp.acts[-1]
        return CurrentAct(phase="aftermath", title="Aftermath", goal="Open-ended play",
                          mood=last.mood, act_number=len(bp.acts), total_acts=len(bp.acts),
                          progress="late")
    ...
    sr = current.scene_range or [1, 20]
    ...
    return CurrentAct(
        phase=current.phase, title=current.title, goal=current.goal,
        scene_range=list(current.scene_range), mood=current.mood,
        transition_trigger=current.transition_trigger,
        act_number=act_number, total_acts=len(acts),
        progress=progress, approaching_end=approaching_end,
    )
...
def get_pending_revelations(game: GameState) -> list[Revelation]:
    ...
    return [
        rev for rev in bp.revelations
        if rev.id not in revealed and game.narrative.scene_count >= rev.earliest_scene
    ]
