...
def should_call_director(game: GameState, roll_result: str = "",
                          chaos_used: bool = False,
                          new_npcs_found: bool = False,
                          revelation_used: bool = False) -> str | None:
    ...
    if bp and bp.acts:
        ...
        if act.phase in ("climax", "resolution", "ten_twist", "ketsu_resolution") and act.phase not in bp.triggered_director_phases:
...
def build_director_prompt(game: GameState, latest_narration: str,
    ...
    for n in game.npcs:
        ...
        if n.status not in ("active", "background"):
            continue
        recent_obs = [m for m in n.memory
                      if m.type == "observation"][-8:]
        mem_text = "; ".join(
            f'{m.event}({m.emotional_weight})' for m in recent_obs
        )
        # Include last reflection so Director can build on it, not repeat it
        prev_reflections = [m for m in n.memory
    ...
    if game.narrative.story_blueprint and game.narrative.story_blueprint.acts:
        ...
        scene_range = act.scene_range or [1, 20]
    ...
    if game.world.clocks:
        ...
        for c in game.world.clocks:
            ...
            line = (
                f'- {c.name} ({c.clock_type}): '
                f'[{bar}] {filled}/{segments} ({pct}%) '
                f'trigger: {c.trigger_description}'
            )
            if c.fired:
...
def call_director(provider: AIProvider, game: GameState,
    ...
    try:
        ...
        response = create_with_retry(
            provider, max_retries=_c.ai.max_retries.director,
            model=_c.ai.director_model, max_tokens=_c.ai.max_tokens.director,
...
def apply_director_guidance(game: GameState, guidance: dict):
    ...
    if guidance.get("act_transition") and game.narrative.story_blueprint:
        ...
        if act.act_number >= total_acts:
            ...
            log(f"[Director] Act transition triggered: act {act.act_number} "
    ...
    for ref in guidance.get("npc_reflections", []):
        ...
        if not reflection_text.rstrip().endswith(('.', '!', '?', '"', '»', '…', ')', '–', '—')):
            log(f"[Director] Rejected truncated reflection for {ref_npc.name}: "
                f"'{reflection_text[:60]}'", level="warning")
            continue

        ref_npc.memory.append(MemoryEntry(
        ...
        if updated_arc:
            if len(updated_arc) > 300:
                log(f"[Director] Rejected arc for {ref_npc.name}: "
                    f"too long ({len(updated_arc)} chars)", level="warning")
            else:
                old_arc = ref_npc.arc
                ref_npc.arc = updated_arc
                log(f"[Director] Arc updated for {ref_npc.name}: "
                    f"'{old_arc[:60]}' → '{updated_arc[:60]}'")
    ...
    for npc in game.npcs:
        if npc.needs_reflection and npc.id not in successfully_reflected_ids:
            npc.needs_reflection = False
            npc.importance_accumulator = 0
            log(f"[Director] Reset stale reflection for {npc.name} "
                f"(accumulator zeroed)")
