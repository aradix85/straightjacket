...
- Principle 3: the web message loop ends the connection on any handler exception, logged but invisible to the player. `IncompatibleSaveError` is now caught at the two load handlers; other handler failures still drop the connection.
