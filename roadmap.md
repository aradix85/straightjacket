...
3. Narrator-prompt tuning for GLM 5.3, measured one change at a time on the twenty harness scenes, judged by GLM 5.3 Fast and by GPT-6 Sol from another family, against `tests/modeltest/baseline_glm.json`, then confirmed with an Elvira run; a difference of about 0.1 overall lies within the noise between two runs of the same prompt (2026.09.25.3). Done in 2026.09.25.3: the miss rule now describes what happens instead of listing what to avoid, which raised result integrity on a miss from 4.53 to 4.83 without moving the overall score or player agency; a broad rewrite of the whole narrator prompt was measured and dropped, because it cost player agency (see the CHANGELOG entry). Open, each to be measured on its own: player agency is the weakest criterion (3.65 out of 5), and the judges mostly fault the narrator for deciding the player character's movements, wounds, and feelings; three rules still apply on a miss without exception (NPCs answer what was asked, one unexplained background detail per scene, a suspended scene ending); the action task tells a strong hit in a desperate phase to carry the surrounding darkness; PLAYER AGENCY describes what a camera could record, including action; `fact_budget` allows one or two extra facts while the dialog task says nothing beyond the question's scope; `<style>` asks for terse prose; most of the system prompt still addresses GENRE PHYSICS; and variable blocks sit in the middle of the system prompt, ahead of fixed rules, which likely costs prefix caching.
