...
Applies to: bugs in touched files regardless of step intent; absolute-rule violations (silent fallbacks, hardcoded domain strings, broad except outside the AI-call carve-out file list, dataclass defaults on config fields, inline imports outside the whitelist in `tests/test_project_rules.py`); comments or docstrings; docs-drift where step contradicts ARCHITECTURE/CHANGELOG; test-hygiene in touched tests (random.seed without teardown, module-state leak).
...
2. Violation grep on touched files: `.get("..", ` on non-neutral literal; `or "..` on domain value; `except Exception` outside the AI-call carve-out files. New hits not present pre-step = new violations, fix.
...
- `tests/test_project_rules.py`: 29 AST/regex scans enforcing absolute rules. Some scans may measure residual debt (acceptable-red). Run before starting a step to see current state; touched-files debt gets fixed in the same commit per the in-scope expansion rule. Three of them are full-codebase consistency scans rather than per-rule pattern checks: `_check_ruff_format_clean` runs `ruff format --check` as a delivery-gate from inside pytest (catches format drift even when pre-commit is not installed locally), `_check_no_orphan_public_symbols` flags any public `def`/`class` in `src/` that has no consumer anywhere with a carve-out for terms truly used via parent-attribute or framework decorators, and `_check_no_orphan_yaml_keys` flags any top-level key in `engine/*.yaml` that has no Python reader (string-literal or dotted-path `get_raw`). The latter two would have caught the `move_routing.yaml` regression and the `drift_checks.py` residue had they existed in 27.10. Since 2026.09.24.1 three more: `_check_no_stale_carve_out_entries` fails on carve-out or whitelist entries that no longer match a file or symbol, `_check_no_skip_or_xfail_in_tests` bans skip/xfail, and `_check_no_orphan_prompt_keys` extends the orphan scan to `prompts/*.yaml`. Since 2026.09.24.2 four documentation-drift scans: paths named in md files exist, the ARCHITECTURE.md file map is complete, ARCHITECTURE.md symbol references resolve, and the CHANGELOG matches the `pyproject.toml` version with strictly decreasing, headed entries. Since 2026.09.24.4 `_check_import_layers` enforces the import layer order described under "Import layers" in ARCHITECTURE.md, and since 2026.09.24.20 `_check_ai_calls_route_by_their_role` requires every AI call to carry its own role name.
- `tests/elvira/elvira_config.yaml`: `full_debug_log: false`. One log file per run.

## Current state

Priorities, in order (set 2026-09-24, revised in 2026.09.24.45):

1. Step 9 (see NEXT STEP below), whose fact resolution reconnects Mythic's fate system, which has had no caller in play since 0.46.50, when the Brain's fate tool went with the move to prompt injection; until step 9 lands, the narrator invents every undetermined fact. The user moved this step to the top because fate is a core part of the design.
2. Open findings from the Elvira runs. Fixed since they were first recorded: the bond-threshold trigger on an id that is not an NPC and the Brain playing moves the situation does not allow (2026.09.24.55), unknown Director tool arguments (.55; now dropped and reported back to the model), Director reflections on NPCs not chosen for reflection (.59), and NPCs following the player to a new place unseen (.60). Still open: the Director sometimes reaches its maximum of three tool rounds and falls back to its last response; the coverage tracker counted no NPC introductions while the metadata extraction reported one; the metadata extraction's identity reveals for unnamed NPCs are rejected for zero word overlap and a stub NPC is created instead (runs of 2026-09-25, on GLM 5.3 Fast among others); one opening-setup extraction failed on unterminated JSON (2026-09-25); and the metadata extraction once named an NPC id that does not exist (`npc_details: could not find NPC 'npc_5'`, 2026.09.25.6). Fixed in 2026.09.25.3: the Brain left a new track's name and rank empty (both are now required in its schema), and a correction to a dialog turn tried to roll the move `dialog` and failed.
3. Narrator-prompt tuning for GPT-6 Luna, the narrator since 2026.09.26.0, measured one change at a time on the twenty harness scenes, judged by GLM 5.3 from another family and by GPT-6 Sol from Luna's own family (whose scores for Luna lean high, 2026.09.24.57), then confirmed with an Elvira run; a difference of about 0.1 overall lies within the noise between two runs of the same prompt (2026.09.25.3). The rounds recorded below were measured on GLM 5.3, so the tuning starts over: first a harness run of the configured narrator against `tests/modeltest/baseline_glm.json`, whose result becomes the Luna baseline, and Luna's own measured weaknesses join the list (narration about half as long as GLM's, atmosphere 4.12 against 4.95, result integrity on a miss 4.20 against 4.58, CHANGELOG 2026.09.24.57). The prompt-level items below were found on GLM and are to be checked again on Luna. Done in 2026.09.25.3: the miss rule now describes what happens instead of listing what to avoid, which raised result integrity on a miss from 4.53 to 4.83 without moving the overall score or player agency; a broad rewrite of the whole narrator prompt was measured and dropped, because it cost player agency (see the CHANGELOG entry). Done in 2026.09.25.4: the narrator system prompt keeps its fixed rules first and the per-turn character state last, for prompt caching, which also raised the overall score from 8.01 to 8.26 and player agency from 3.65 to 3.89; a 40 percent shorter rule-list version of the same rules was measured and dropped (8.00, the other-family judge 0.5 lower). Open, each to be measured on its own: player agency is the weakest criterion (3.65 out of 5), and the judges mostly fault the narrator for deciding the player character's movements, wounds, and feelings; three rules still apply on a miss without exception (NPCs answer what was asked, one unexplained background detail per scene, a suspended scene ending); the action task tells a strong hit in a desperate phase to carry the surrounding darkness; PLAYER AGENCY describes what a camera could record, including action; `fact_budget` allows one or two extra facts while the dialog task says nothing beyond the question's scope; `<style>` asks for terse prose; and most of the system prompt still addresses GENRE PHYSICS. Beyond the narrator: `prompts/director.yaml` still has the Director recommend a "breather" although pacing is engine-computed, and forbids gothic horror imagery in every setting; the extraction prompts (`narrator_metadata`, `opening_setup_extractor`), `revelation_check_system`, `blueprint_voicing`, `chapter_summary`, and `recap` were read in 2026.09.25 but not yet tuned. The harness measures only the narrator; the other roles have no measurement besides Elvira, which a round on their prompts needs first. The Director is also the largest cost: about half of a session (22 calls and 34 tool rounds in eight turns, 2026.09.25.5), because it runs after most turns with up to three tool rounds that each resend the whole context, and the next turn waits for it. Running it less often would leave NPC profiles stale and is not planned; fewer tool rounds, or prompt-injecting what it now queries, are the candidates, to be measured once the Director has a measurement of its own.
4. Test fixtures made through the real character-creation path instead of hand-built states, since a hand-built fixture hid the roll-bonus bug of 2026.09.24.33. The strict loader of 2026.09.24.45 already forced the Brain mocks and several hand-built dicts into the shape production uses.
5. Smaller open items from the review of 2026-09-25, each small: the Brain prompt routes player questions ("is the door locked?") to `ask_the_oracle`, while ARCHITECTURE.md says the player types actions, never questions, a design question to settle with step 9; `web/serializers.py` → `build_creation_options` still branches on the setting id `delve` (Principle 2 material, see AUDIT.md); steps 12.3 and 15.2 below still say only "templates in yaml", which should read `prompts/blocks.yaml` per the Yaml content boundary; a short summary at the top of CHANGELOG.md would make its 300-odd kilobytes navigable with a screen reader; no principle audit of AUDIT.md has been run yet (every status is todo); and Elvira's judge is GPT-6 Luna judging its own model's prose, while the harness also judges with GLM 5.3 from another family: putting GLM on Elvira's judge would make her audit independent (GPT-6 Sol would not, being Luna's family), but needs her judge to name its own provider and model again, as it did from 2026.09.24.49 until 2026.09.24.57 made it follow the Brain.

Working agreements (2026-09-24): commit only when ruff check, ruff format, mypy --strict, and the full test suite all pass; write nothing in the CHANGELOG before its check has been read; run a short Elvira run (8 to 20 turns, about one cent per eight turns on GPT-6 Luna at list price) before a release that touches the turn pipeline, AI calls, prompts, or configuration, and treat every engine warning in its report as a finding; prefer fewer, larger releases over many small ones.

Running things: API keys live in the Windows user environment (OPENAI_API_KEY runs the game; FIREWORKS_API_KEY is needed only for the harness's GLM judge). Elvira runs as `python tests/elvira/elvira.py --auto` (eight turns, setting and play style at random, one injected narrator outage that must roll back cleanly, the game's own model as player and judge); `--matrix` plays all nine setting and style combinations, `--scenario all` plays the prepared rare situations (near death, chapter end, momentum burn, combat, a filling clock), `--turns 40` makes a long run, and `--ws` plays through the server. It writes its JSON log and Markdown report to `tests/elvira/runs/`, which git ignores. The key must be in the environment of the process that starts it. The model-comparison harness is `tests/modeltest/` (since 2026.09.24.50): `python tests/modeltest/modeltest.py --models current` measures the configured narrator on twenty fixed scenes, three attempts each, judged blind by GLM 5.3 and GPT-6 Sol, and compares with `baseline_glm.json`; `--capture` records the scenes again after a narrator-prompt change, which the drift tests in `tests/test_modeltest.py` demand.
...
Models and providers (2026.09.26.0): every AI role and Elvira's player and judge run on GPT-6 Luna through OpenAI's own API, chosen for speed and cost over GLM 5.3's richer narration, at reasoning effort `none` except the creative cluster at `low` (the GPT-6 rules behind that split are in ARCHITECTURE.md → AI Model Assignment); set in each cluster in `config.yaml`. The harness keeps GLM 5.3 through Fireworks as its judge from another family, beside GPT-6 Sol. GLM 5.3 through Fireworks, the game's model from 2026.09.24.61 to 2026.09.25.6 (the Fast tier, then the standard tier), is the measured alternative, and Gemini 3.8 Flash, best through Google's own API, the other one. The comparisons behind these choices are in CHANGELOG 2026.09.24.56 to .60 and 2026.09.25.5 to .6.
...
Principle (2026-09-24): a rule taken from Ironsworn/Starforged, Mythic GME 2e, the Adventure Crafter, or Blades in the Dark works as its source describes, or is listed under "Deliberate divergences from the source rulebooks" in ARCHITECTURE.md. `tests/test_rules_conformance.py` pins what has been checked, including automated comparisons with the Datasworn move texts that fail on any new divergence.

Checked and conform or fixed (2026.09.24.17 to .34): the action roll (one d6), negative momentum, momentum values and burn, momentum per outcome, Endure Harm and Endure Stress per rulebook, Pay the Price from the official tables, every match clause, chained moves including oracle moves, progress ticks, legacy tracks and experience, connections, the Mythic fate check, fate chart, scene test, chaos factor, event focus, lists, and meaning-table shape, the Adventure Crafter tables, theme priority, and turning points, Blades clock sizes, and asset and connection adds.

Open: outcomes where the player chooses (the engine takes fixed choices, recorded in ARCHITECTURE.md); ability effects other than adds (step 18); wounded and shaken blocking recovery in classic Ironsworn; the individual words of the oracle and meaning tables, not checked against the books.
...
Datasworn defines 89 unique move stems in the move categories of the four shipped settings; 56 are implemented as formal moves in `engine/move_outcomes.yaml` (recounted in 2026.09.24.46). The other 33 are all `no_roll` (29) or `special_track` (4): 6 are setting-specific or step-bound (covered by steps 25, 26, 28), and roughly 27 are general-mechanic moves not yet wired. Assets define further moves inside their abilities (48 across the four files, counted per setting), covered by steps 18 to 20; the earlier figure of 122 stems apparently counted some of those with the core moves. Step 13b wires the general-mechanic group following the "Datasworn mechanic naming" Key Design Decision in ARCHITECTURE.md: a move that represents a player choice with structured outcome lands as a formal move in `move_outcomes.yaml`; a move that fires automatically when a mechanical condition becomes true lands as an engine-trigger with a direct name like `advance_menace_on_miss`.
...
**14b.3** Datasworn faction oracles for generation: pull faction archetypes per setting from `data/settings/*.yaml::oracle_paths.factions` (the Python field was removed in 27.9 because no consumer existed yet, and the setting-yaml keys followed in 2026.09.25.0, when the settings loader began refusing unknown keys; step 14b is the consumer that justifies their return). Add `factions: str` back to the `OraclePaths` dataclass and the paths back to the setting yamls in this step's commit (starforged `factions`, sundered_isles `faction`; classic Ironsworn has no faction oracle); missing path on a setting that has factions = KeyError per strict-rules.
...
State (2026.09.24.33 and .41): `GameState.asset_abilities` records the enabled abilities of every path and asset (Datasworn defaults; an upgrade enables the next ability in order), and enabled abilities with an add reach the action roll through the Brain's `bonus_id`, with the number taken from the rule text (`mechanics/assets.py`, `mechanics/bonuses.py`). The earlier plan for a typed `AssetState` with a four-type modifier pipeline predates this and is superseded; the steps below build on the existing mechanism.

**18.1** Rerolls granted by abilities ("you may reroll any dice").
...
**22.1** NPC-to-player requests. Engine generates from NPC state (disposition, bond, recent events). Rules in `engine/emotional_requests.yaml` (new); narrator-facing templates in `prompts/blocks.yaml`, per the Yaml content boundary.
...
**28.2** Cursed die. The earlier sketch here (the `cursed` impact forcing the lowest of two challenge dice) does not match the source: the Datasworn texts that mention the cursed die, all in the Chattering Skull asset, describe an extra ten-sided die added to the roll, with an effect when it shows a 10. Check the Sundered Isles rules for when the cursed die is rolled before designing this, per section R. The `cursed` impact in `engine/impacts.yaml` already exists (2026.09.24.34) and is a separate mechanic.
