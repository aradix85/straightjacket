...
1. Step 9 (see NEXT STEP below), whose fact resolution reconnects Mythic's fate system. Mythic's fate system has had no caller in play since 0.46.50, when the Brain's fate tool went with the move to prompt injection; until step 9 lands, the narrator invents every undetermined fact. The user moved this step to the top because fate is a core part of the design.
...
3. Narrator-prompt tuning for GPT-6 Luna: honest failures (one miss narrated as a success, one strong hit given extra pressure) and player agency. Measure on the ten test situations with the two blind judges, then confirm with an Elvira run.
