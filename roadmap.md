...
Running things: API keys live in the Windows user environment (ANTHROPIC_API_KEY, OPENAI_API_KEY, and others). Elvira runs as `python tests/elvira/elvira.py --auto` (eight turns, setting and play style at random, one injected narrator outage that must roll back cleanly, Claude Sonnet 5 as judge); `--matrix` plays all nine setting and style combinations, `--turns 40` makes a long run, and `--ws` plays through the server. It writes its JSON log and Markdown report to `tests/elvira/runs/`, which git ignores. Both API keys must be in the environment of the process that starts it. The model-comparison harness is `tests/modeltest/` (since 2026.09.24.50): `python tests/modeltest/modeltest.py --models current` measures the configured narrator on ten fixed scenes, three attempts each, judged blind by GPT-6 Sol and Claude Sonnet 5, and compares with `baseline_gpt-6-luna.json`; `--capture` records the scenes again after a narrator-prompt change, which the drift tests in `tests/test_modeltest.py` demand. The old folder `C:\Users\radix\Documents\sj_modeltest` is an archive of the 24 September measurements.
