...
2. Open findings from the Elvira runs. Fixed since they were first recorded: the bond-threshold trigger on an id that is not an NPC and the Brain playing moves the situation does not allow (2026.09.24.55), unknown Director tool arguments (.55; now dropped and reported back to the model), Director reflections on NPCs not chosen for reflection (.59), and NPCs following the player to a new place unseen (.60). Still open: the Director sometimes reaches its maximum of three tool rounds and falls back to its last response; the coverage tracker counted no NPC introductions while the metadata extraction reported one; the metadata extraction's identity reveals for unnamed NPCs are rejected for zero word overlap and a stub NPC is created instead (runs of 2026-09-25, on GLM 5.3 Fast among others); one opening-setup extraction failed on unterminated JSON (2026-09-25); and the Brain still sometimes leaves a new combat track's name and rank empty, although `prompts/brain.yaml` explains both since 2026.09.24.60 (seen again in the Elvira run of 2026.09.25.2).
3. Narrator-prompt tuning for GLM 5.3: atmosphere, honest failures (a miss narrated as a success, a strong hit given extra pressure), and player agency. Measure on the twenty test situations with the harness, judged by GLM itself and compared with `tests/modeltest/baseline_glm.json`, then confirm with an Elvira run. Measurement caveat (2026.09.25.0): the harness baseline gives GLM, judged by itself, 5 out of 5 for result integrity on a miss, where the two-judge panel of 2026.09.24.57 gave the same model 4.58, so a gain on the property this priority targets cannot show in that measurement. The Elvira sessions on GLM of 2026-09-25 report no silver lining on a miss, while that day's sessions with GPT-6 Luna as narrator and judge report several; from these runs it cannot be told whether the narrator or the judge makes the difference. Before tuning, decide between scenes that reproduce the failure and a judge from another family (2026.09.24.57 found that judges favour their own family). The outright errors in the prompts were fixed in 2026.09.25.2 (see there). What remains is the measured round, input from the prompt review of 2026.09.25: the MISS rule and the per-turn miss instruction name what they want to avoid ("no silver linings"), which the design document's constraint-writing principles and the lesson of 2026.04.27.4 advise against; three rules apply on a miss without exception and supply exactly the useful finding the judges flag (NPCs must answer what was asked, one unexplained background detail per scene, a suspended scene ending); the action task tells a strong hit in a desperate phase to carry the surrounding darkness, a likely source of strong hits given a complication; PLAYER AGENCY says to describe what a camera could record, including action, which invites actions the player never chose; `fact_budget` allows one or two extra facts while the dialog task says nothing beyond the question's scope; `<style>` asks for terse prose against the atmosphere the user asked for in 2026.09.24.57; most of the system prompt still addresses GENRE PHYSICS, the weakness of GLM 4.7 and Qwen; and variable blocks sit in the middle of the system prompt, ahead of fixed rules, which likely costs prefix caching. `tests/modeltest/baseline_glm.json` was narrated with the prompts of before 2026.09.25.2 and should be renewed at the start of this round.
