...
5. Smaller open items from the review of 2026-09-25, each small: the Brain prompt routes player questions ("is the door locked?") to `ask_the_oracle`, while ARCHITECTURE.md says the player types actions, never questions, a design question to settle with step 9; `web/serializers.py` → `build_creation_options` still branches on the setting id `delve` (Principle 2 material, see AUDIT.md); steps 12.3 and 15.2 below still say only "templates in yaml", which should read `prompts/blocks.yaml` per the Yaml content boundary; no principle audit of AUDIT.md has been run yet (every status is todo); Elvira's player and judge name their own provider and model again since 2026.09.26.2 (GPT-6 Luna), so while the game also runs on Luna the judge scores its own model's prose, and a judge from another model family would make that audit independent; the judge also sees one turn at a time, so it calls continuity from earlier turns invented, and it marks down information on a miss even where Ironsworn's miss outcome is an unwelcome truth (2026.09.26.2); and the correction schema still takes an NPC's `status` as free text: 2026.09.26.2 limited `disposition` to the known dispositions after a model wrote a stance there and the stance matrix crashed, but the engine has no single list of NPC statuses to limit `status` to.
...
Running things: API keys live in the Windows user environment (OPENAI_API_KEY runs the game and Elvira). Elvira runs as `python tests/elvira/elvira.py --auto` (eight turns, setting and play style at random, one injected narrator outage that must roll back cleanly, the game's own model as player and judge); `--matrix` plays all nine setting and style combinations, `--scenario all` plays the prepared rare situations (near death, chapter end, momentum burn, combat, a filling clock), `--turns 40` makes a long run, and `--ws` plays through the server. It writes its JSON log and Markdown report to `tests/elvira/runs/`, which git ignores. The key must be in the environment of the process that starts it.
