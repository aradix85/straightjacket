...
**R.1 Action roll details.** Checked 2026-09-24 (2026.09.24.21). Momentum burn conforms: only on a miss or weak hit, only with positive momentum, never on progress rolls, momentum resets afterwards, and a match stays a match. Negative momentum cancelling a matching action die was missing and is now implemented with tests. Still open: adds from assets and moves (+1 or +2 under asset-specific conditions) are not applied at all; this needs the Brain or the engine to decide when an asset's condition holds, so it is a feature, not a fix. The move effect `next_move_bonus` (for example Starforged's Secure an Advantage strong hit, "add +1 on your next move") now works (2026.09.24.23): it is banked on the character and added to the next action roll, never to a progress roll, so the action roll now carries adds. Asset and connection adds followed in 2026.09.24.33: the engine tracks which abilities of each asset are enabled (Datasworn defaults, one more per upgrade), offers every enabled ability with an add and every connection's aid to the Brain, and applies the one the Brain names after checking it; live, the Brain picked Combat Bot for a Strike aided by the bot and nothing for studying a star chart. R.1 is complete for adds; ability effects other than adds remain unmodelled. Per-setting overrides exist since 2026.09.24.24: `move_outcome_overrides` in `engine/move_outcomes.yaml` gives classic Ironsworn its own Secure an Advantage, Endure Harm, and Endure Stress.
...
**R.3 Suffer moves and impacts.** Endure Harm and Endure Stress checked 2026-09-24 against both rulebooks. Starforged conformed except a weak hit, which traded momentum for health even at full health; the exchange now happens only when it recovers something. Classic Ironsworn now has its own numbers (shake it off costs 1 momentum, a weak hit presses on, a miss costs 1 momentum), pinned per case in `tests/test_rules_conformance.py`. Pay the Price checked and fixed 2026-09-24 (2026.09.24.29): the engine drew one of eight invented lines inherited from EdgeTales and applied no cost; it now rolls the setting's own Datasworn table (twenty rows in Starforged, sixteen in classic Ironsworn), follows its roll-twice and roll-again rows, and applies a cost of 1 for the harmed, stressed, and wasted-resources rows. An automated check of the Datasworn outcome texts for suffer moves, Pay the Price, and progress found one real divergence, fixed: classic Enter the Fray on a miss also pays the price. Match outcomes added 2026-09-24 (2026.09.24.30): the Datasworn texts hold seven "with a match" clauses, all in Starforged; the four mechanical ones (scene challenge Face Danger and Secure an Advantage, on a strong hit and on a miss) are now `strong_hit_match` and `miss_match` entries that replace the plain outcome on a match. The remaining three followed in 2026.09.24.32: Explore a Waypoint's match clauses chain the oracle moves Make a Discovery and Confront Chaos, and Develop Your Relationship's match raises the connection's rank. Every match clause in the Datasworn texts is now modelled, and `tests/test_rules_conformance.py` fails on any new one that is not.

**R.4 Progress.** Ticks per rank checked 2026-09-24 and conform (troublesome 12, dangerous 8, formidable 4, extreme 2, epic 1, on 40-tick tracks); progress rolls use filled boxes against two d10s without momentum. Legacy and connections checked against the Starforged Datasworn texts (2026.09.24.25): legacy rewards per rank and 2 experience per box conformed; fixed were Make a Connection (no momentum or progress), Develop Your Relationship (a fixed 2 bonds ticks), Fulfill Your Vow on a weak hit (one rank lower, since the engine cannot know whether a new vow is sworn), and a full legacy track (cleared and continued at 1 experience per box instead of stopping). Test Your Relationship now chains Develop Your Relationship, rolled in the same turn with the connection's rank (2026.09.24.31). Still open: a connection's aid bonus (add +1 and +1 momentum when they help on a move matching their role), which belongs with asset adds.
...
**R.8 Adventure Crafter.** Theme priority checked 2026-09-24 and conforms (d10: 1 to 4 first theme, 5 to 7 second, 8 and 9 third, 10 alternating fourth and fifth). Tables checked 2026-09-24 (2026.09.24.27) and complete: for each of the five themes the plot points cover 1 to 100 without gaps or overlaps, as do the meta plot points; Conclusion sits at 1 to 8 and None at 9 to 24 on every theme; the character and plotline lists have 25 lines of 4 percent each. Turning-point assembly follows the rules summarised in `data/adventure_crafter.json` (plotline by d100, a theme die plus d100 per plot point, Conclusion flipping the plotline). Turning-point assembly fixed 2026-09-24 (2026.09.24.28) after finding the rulebook text: every turning point rolls five plot points, at most three may be None and a fourth None is rerolled, and a Conclusion on a new plotline or one already concluding counts as None. The engine drew a random 2 to 5 plot points instead, so a turning point could end with none at all, and a Conclusion could close a plotline the moment it began. R.8 is complete.
