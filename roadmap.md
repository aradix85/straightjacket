...
Running things: API keys live in the Windows user environment (FIREWORKS_API_KEY runs the game). Elvira runs as `python tests/elvira/elvira.py --auto` (eight turns, setting and play style at random, one injected narrator outage that must roll back cleanly, the game's own model as player and judge); `--matrix` plays all nine setting and style combinations, `--scenario all` plays the prepared rare situations (near death, chapter end, momentum burn, combat, a filling clock), `--turns 40` makes a long run, and `--ws` plays through the server. It writes its JSON log and Markdown report to `tests/elvira/runs/`, which git ignores. The key must be in the environment of the process that starts it. The model-comparison harness is `tests/modeltest/` (since 2026.09.24.50): `python tests/modeltest/modeltest.py --models current` measures the configured narrator on twenty fixed scenes, three attempts each, judged blind by GLM 5.3 Fast and GPT-6 Sol, and compares with `baseline_glm.json`; `--capture` records the scenes again after a narrator-prompt change, which the drift tests in `tests/test_modeltest.py` demand.
...
Models and providers (2026.09.24.61): every AI role, Elvira's player and judge, and the harness judge run on GLM 5.3 Fast through Fireworks at reasoning effort low, set in each cluster in `config.yaml`; see ARCHITECTURE.md → AI Model Assignment. Gemini 3.8 Flash, best through Google's own API, is the measured alternative. The comparisons behind the choice are in CHANGELOG 2026.09.24.56 to .60.
