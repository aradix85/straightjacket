...
Models and providers (2026.09.25.6): every AI role and Elvira's player and judge run on GLM 5.3 through Fireworks' standard tier at reasoning effort low, chosen for cost over waiting time; so does the harness's GLM judge beside GPT-6 Sol; set in each cluster in `config.yaml`; see ARCHITECTURE.md → AI Model Assignment. Gemini 3.8 Flash, best through Google's own API, is the measured alternative. The comparisons behind the choice are in CHANGELOG 2026.09.24.56 to .60.
