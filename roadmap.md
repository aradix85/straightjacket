...
3. Narrator-prompt tuning for GLM 5.3: atmosphere, honest failures (a miss narrated as a success, a strong hit given extra pressure), and player agency. Measure on the twenty test situations with the harness, judged by GLM itself and compared with `tests/modeltest/baseline_glm.json`, then confirm with an Elvira run. Input from the prompt check in 2026.09.24.46, for the same measured round: `prompts/narrator.yaml` still tells the model that violations "require a rewrite", a retry loop removed in 2026.04.27.8; it names what it wants to avoid (the MISS rule lists "silver linings", the patterns block lists the forbidden verbs word for word and seven WRONG examples of player agency), which the design document's constraint-writing principles and the lesson of 2026.04.27.4 advise against, and the only audit problem in the Elvira run of 2026.09.24.45 was a silver lining on a miss; `opening_setup_extractor` asks for `bond` and `bond_max`, which the schema has not had since 0.46.0.
...
Models and providers (2026.09.24.61): every AI role, Elvira's player and judge, and the harness judge run on GLM 5.3 Fast through Fireworks at reasoning effort low, set once in `ai.model` in `config.yaml`; see ARCHITECTURE.md → AI Model Assignment. Gemini 3.8 Flash, best through Google's own API, is the measured alternative. The comparisons behind the choice are in CHANGELOG 2026.09.24.56 to .60.
