...
5. Smaller open items from the review of 2026-09-25, each small: the Brain prompt routes player questions ("is the door locked?") to `ask_the_oracle`, while ARCHITECTURE.md says the player types actions, never questions, a design question to settle with step 9; `web/serializers.py` → `build_creation_options` still branches on the setting id `delve` (Principle 2 material, see AUDIT.md); steps 12.3 and 15.2 below still say only "templates in yaml", which should read `prompts/blocks.yaml` per the Yaml content boundary; no principle audit of AUDIT.md has been run yet (every status is todo); Elvira's player and judge name their own provider and model since 2026.09.26.2 (GPT-6 Luna), a family apart from the game's GLM 5.3 Flash since 2026.09.26.5, so her audit no longer scores its own model; the judge also sees one turn at a time, so it calls continuity from earlier turns invented, and it marks down information on a miss even where Ironsworn's miss outcome is an unwelcome truth (2026.09.26.2).

Working agreements (2026-09-24): commit only when ruff check, ruff format, mypy --strict, and the full test suite all pass; write nothing in the CHANGELOG before its check has been read; run a short Elvira run (8 to 20 turns, about two to three cents per eight turns with the game on GLM 5.3 Flash and Elvira on GPT-6 Luna) before a release that touches the turn pipeline, AI calls, prompts, or configuration, and treat every engine warning in its report as a finding; prefer fewer, larger releases over many small ones.

Running things: API keys live in the Windows user environment (TOGETHER_API_KEY runs the game, OPENAI_API_KEY Elvira). Elvira runs as `python tests/elvira/elvira.py --auto` (eight turns, setting and play style at random, one injected narrator outage that must roll back cleanly, the game's own model as player and judge); `--matrix` plays all nine setting and style combinations, `--scenario all` plays the prepared rare situations (near death, chapter end, momentum burn, combat, a filling clock), `--turns 40` makes a long run, and `--ws` plays through the server. It writes its JSON log and Markdown report to `tests/elvira/runs/` and keeps her players in `tests/elvira/users/`, both ignored by git. The key must be in the environment of the process that starts it.
...
Models and providers (2026.09.26.5): every AI role runs on GLM 5.3 Flash through Together's own API, at reasoning effort `low` (it always thinks) with a fixed `user` value for the cache, the user's choice for its narration over GPT-6 Luna's speed and cost; Elvira's player and judge stay on GPT-6 Luna. GPT-6 Luna (the game's model in 2026.09.26.0 and from 2026.09.24.36 to .60) is the measured alternative for speed and cost; hosts and cheaper models measured for GLM 5.3 Flash are in CHANGELOG 2026.09.26.2 to .5. Fireworks, OpenRouter, and OpenAI stay configured; Baseten needs a verified account (15 requests per minute otherwise) and a session header the adapter does not send.
