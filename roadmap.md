...
2. Open findings from the Elvira runs of 2026.09.24.38 to .42. The Director sometimes hits its maximum of three tool rounds and falls back to its last response (a warning in the report). The coverage tracker counted no NPC introductions while the metadata extraction reported one, so the two counts disagree. On a location change two NPCs were moved to the new location without the narration placing them there (spatial check); the scenario runs of 2026.09.24.51 showed it again, three times in one session. New in that release: the Director called `query_active_threads` with an argument the tool does not take (`threads`), and the call failed with a warning; the tool schemas should forbid extra arguments, or the executor should report them back to the model. Found by the narrator sessions of 2026.09.24.54: the combat position can outlive its combat track (Elvira's combat-sync invariant fired on three turns in a row), and a bond-threshold trigger referred to an NPC id `char_1` that does not exist, which raised and ended the session.
3. Narrator-prompt tuning for GPT-6 Luna: honest failures (one miss narrated as a success, one strong hit given extra pressure) and player agency. Measure on the ten test situations with the two blind judges, then confirm with an Elvira run. Input from the prompt check in 2026.09.24.46, for the same measured round: `prompts/narrator.yaml` still tells the model that violations "require a rewrite", a retry loop removed in 2026.04.27.8; it names what it wants to avoid (the MISS rule lists "silver linings", the patterns block lists the forbidden verbs word for word and seven WRONG examples of player agency), which the design document's constraint-writing principles and the lesson of 2026.04.27.4 advise against, and the only audit problem in the Elvira run of 2026.09.24.45 was a silver lining on a miss; `opening_setup_extractor` asks for `bond` and `bond_max`, which the schema has not had since 0.46.0.
...
Working agreements (2026-09-24): commit only when ruff check, ruff format, mypy --strict, and the full test suite all pass; write nothing in the CHANGELOG before its check has been read; run a short Elvira run (8 to 20 turns, a few cents on GPT-6 Luna) before a release that touches the turn pipeline, AI calls, prompts, or configuration, and treat every engine warning in its report as a finding; prefer fewer, larger releases over many small ones.

Running things: API keys live in the Windows user environment (ANTHROPIC_API_KEY, OPENAI_API_KEY, and others). Elvira runs as `python tests/elvira/elvira.py --auto` (eight turns, setting and play style at random, one injected narrator outage that must roll back cleanly, the game's own model as player and judge; `--model <name>` puts a whole session on a contestant); `--matrix` plays all nine setting and style combinations, `--scenario all` plays the prepared rare situations (near death, chapter end, momentum burn, combat, a filling clock), `--turns 40` makes a long run, and `--ws` plays through the server. It writes its JSON log and Markdown report to `tests/elvira/runs/`, which git ignores. Both API keys must be in the environment of the process that starts it. The model-comparison harness is `tests/modeltest/` (since 2026.09.24.50): `python tests/modeltest/modeltest.py --models current` measures the configured narrator on twenty fixed scenes, three attempts each, judged blind by a panel (GPT-6 Sol and Grok 4.7), and compares with `baseline_luna_gemini_sol.json`; `--capture` records the scenes again after a narrator-prompt change, which the drift tests in `tests/test_modeltest.py` demand. The old folder `Documents\sj_modeltest` went to the recycle bin in 2026.09.24.51.
...
Models and providers (2026.09.24.42): every AI role runs on OpenAI's GPT-6 Luna, the Director in its own cluster; see ARCHITECTURE.md → AI Model Assignment. The comparisons behind the choice (Opus 5.5, Sonnet 5, Haiku 4.5, GPT-6 Luna, and others on ten situations with two blind judges) and the day's sequence of decisions are in CHANGELOG 2026.09.24.11 to .36; the harness is `tests/modeltest/`. Still to check by the user: how NVDA reads the streamed sentences (`server.stream_narration: false` switches streaming off). Next for Luna: narrator-prompt tuning for honest failures and player agency, measured on the same ten situations.
Model choice, state of 2026.09.24.59: the user wants one model for every role, including Elvira's player and judge, and leans to GLM 5.3 Fast through Fireworks (reasoning effort low; fastest measured route for GLM 5.3, about 125 tokens per second under load; best-scoring narrator with Gemini 3.8 Flash on the panel; clean over nine whole-game sessions after 2026.09.24.58 and .59). Gemini 3.8 Flash is the alternative, best taken through Google's own API. Before switching: put the chosen model in `config.yaml` as one main cluster that every role uses, with room for per-role exceptions; add the Fireworks provider there; then tune the narrator prompt (atmosphere, honest failure) with the harness judging on the chosen model. Open: GLM's Brain sometimes leaves a new track's name and rank empty; Elvira's spatial check still flags a place re-described with many details; the 37 OpenRouter providers for GLM 5.3 were not each measured.
