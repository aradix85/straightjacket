...
def test_snapshot_with_brain_roundtrip():
    ...
    snap.brain = BrainResult(
        move="face_danger", stat="wits", position="desperate",
        effect="great", player_intent="Search carefully",
        target_npc="npc_1", dramatic_question="Will they find it?",
    )
    d = snap.to_dict()
...
def test_snapshot_with_roll_roundtrip():
    ...
    snap.roll = RollResult(
        d1=4, d2=3, c1=6, c2=8, stat_name="iron",
        stat_value=3, action_score=10, result="STRONG_HIT",
        move="strike", match=False,
    )
    d = snap.to_dict()
...
def test_snapshot_corrupted_roll_recovers():
    ...
    d = {
        "resources": {}, "world": {}, "narrative": {}, "campaign": {},
        "npcs": [], "crisis_mode": False, "game_over": False,
        "player_input": "test",
        "roll": {"garbage": True},  # missing required fields
        "brain": None, "narration": None,
    }
    restored = TurnSnapshot.from_dict(d)
...
def test_snapshot_json_full_roundtrip():
    ...
    snap = TurnSnapshot(
        resources={"health": 4, "spirit": 3, "supply": 5,
                    "momentum": 6, "max_momentum": 10},
        world={"current_location": "Forest", "chaos_factor": 5,
               "current_scene_context": "Dark", "time_of_day": "night",
               "location_history": ["Town"], "clocks": []},
        narrative={"scene_count": 5, "director_guidance": {},
                   "scene_intensity_history": ["action"]},
        campaign={"epilogue_shown": False, "epilogue_dismissed": False},
        npcs=[{"id": "npc_1", "name": "Mira", "bond": 2}],
        crisis_mode=False, game_over=False,
        player_input="I hide behind the tree",
    )
    snap.brain = BrainResult(move="face_danger", stat="shadow", position="risky")
    snap.roll = RollResult(d1=5, d2=2, c1=3, c2=9, stat_name="shadow",
                            stat_value=2, action_score=9, result="WEAK_HIT",
...
def test_gamestate_snapshot_last_turn_persists():
    ...
    game.last_turn_snapshot.roll = RollResult(
        d1=6, d2=6, c1=3, c2=4, stat_name="iron", stat_value=3,
        action_score=10, result="STRONG_HIT", move="strike", match=False)
    d = game.to_dict()
...
def test_npc_id_remapping():
    ...
    returning = [
        NpcData(id="npc_1", name="Kira", bond=3,
                memory=[MemoryEntry(scene=1, event="old memory", about_npc="npc_2")]),
        NpcData(id="npc_2", name="Borin", bond=1,
                memory=[MemoryEntry(scene=1, event="another memory", about_npc="npc_1")]),
    ]
    # Simulate the merge: assign fresh IDs to returning NPCs
    new_npc_names = {n.name.lower().strip() for n in game.npcs}
    id_remap = {}
    next_id = max(
        int(n.id.split("_")[1]) for n in game.npcs if n.id.startswith("npc_")
    ) + 1
...
def test_story_blueprint_roundtrip():
    ...
    bp = StoryBlueprint.from_dict({
        ...
        "acts": [
            ...
            {"phase": "confrontation", "title": "Darkness",
             "goal": "Face it", "scene_range": [8, 14],
             "mood": "tense", "transition_trigger": "Shadow revealed"},
        ],
        "revelations": [
            {"id": "rev_1", "content": "The shadow is sentient",
             "earliest_scene": 5, "dramatic_weight": "high"},
        ],
        "possible_endings": [
...
def test_gamestate_with_blueprint_roundtrip():
    ...
    game.narrative.story_blueprint = StoryBlueprint.from_dict({
        ...
        "acts": [
            {"phase": "ki_introduction", "title": "Daily Life",
             "goal": "Establish", "scene_range": [1, 5],
             "mood": "contemplative", "transition_trigger": "Disruption"},
        ],
        "revelations": [
            {"id": "r1", "content": "Hidden truth",
             "earliest_scene": 3, "dramatic_weight": "critical"},
        ],
