...
def test_npcdata_roundtrip():
    npc = NpcData(id="npc_1", name="Kira", bond=3,
                  memory=[MemoryEntry(event="test", type="observation")],
                  aliases=["K"])
    d = npc.to_dict()
...
def test_memoryentry_roundtrip():
    m = MemoryEntry(scene=3, event="fought", emotional_weight="angry",
                    importance=5, type="observation", about_npc="npc_1",
                    tone="defiant_rage", tone_key="angry")
    d = m.to_dict()
...
def test_scenelogentry_roundtrip():
    e = SceneLogEntry(scene=3, summary="Test", result="STRONG_HIT",
...
def test_chapter_summary_roundtrip():
    cs = ChapterSummary(
        chapter=1, title="First Blood", summary="It began.",
...
def test_campaign_state_from_dict_converts_chapter_dicts():
    ...
    data = {
        "campaign_history": [
            {"chapter": 1, "title": "Old Save", "summary": "From JSON",
             "unresolved_threads": ["thread"], "character_growth": "",
             "thematic_question": "", "post_story_location": "", "scenes": 10,
...
def test_campaign_state_roundtrip():
    cs = CampaignState(
        campaign_history=[
            ...
            ChapterSummary(chapter=2, title="C", summary="D",
                           npc_evolutions=[NpcEvolution(name="X", projection="Y")]),
        ],
        chapter_number=3,
    )
    d = cs.to_dict()
...
def test_gamestate_full_json_roundtrip():
    ...
    game.npcs.append(NpcData(id="npc_1", name="Ally", bond=2,
                             memory=[MemoryEntry(event="met")]))
...
def test_compel_no_disposition_shift():
    ...
    roll = RollResult(d1=5, d2=5, c1=2, c2=3, stat_name="heart", stat_value=2,
                      action_score=10, result="STRONG_HIT", move="compel")
...
def test_test_bond_disposition_shift():
    ...
    game.npcs.append(npc)
    roll = RollResult(d1=5, d2=5, c1=2, c2=3, stat_name="heart", stat_value=2,
                      action_score=10, result="STRONG_HIT", move="test_bond")
    brain = BrainResult(target_npc="npc_1", effect="standard")
    apply_consequences(game, roll, brain)
    assert npc.bond == 2
...
def test_story_blueprint_triggered_director_phases():
    ...
    bp = StoryBlueprint(
        ...
        revealed=[], triggered_transitions=[], story_complete=False,
    )
...
def test_story_blueprint_triggered_director_phases_snapshot():
    ...
    game = GameState()
    bp = StoryBlueprint(
        central_conflict="c", antagonist_force="a",
        thematic_thread="t", structure_type="3act",
        revealed=[], triggered_transitions=[], story_complete=False,
    )
...
def test_phase_trigger_dedup():
    ...
    bp = StoryBlueprint(
        central_conflict="c", antagonist_force="a",
        thematic_thread="t", structure_type="3act",
        revealed=[], triggered_transitions=["act_0"], story_complete=False,
    )
    bp.acts = [
...
def test_memory_guard_rejects_zero_overlap():
    ...
    game = GameState()
    npc = NpcData(
        id="npc_1", name="Theo", status="active",
...
def test_social_move_unresolved_target_skips_bond():
    ...
    game = GameState()
    roll = RollResult(d1=1, d2=1, c1=10, c2=10, stat_name="heart",
                      stat_value=2, action_score=4, result="MISS",
                      move="compel", match=False)
