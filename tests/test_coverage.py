...
from straightjacket.engine.models import (
    ClockData, GameState, NpcData, SceneLogEntry, StoryAct, StoryBlueprint,
)
...
def _stub():
    engine_loader._eng = _ConfigNode({
        ...
        "npc": {"max_active": 12, "reflection_threshold": 30,
                ...
                "max_activated": 3},
        "activation_scores": {"target": 1.0, "name_match": 0.8, "name_part": 0.6,
        ...
        "move_categories": {"combat": ["clash"], "social": ["compel"],
                            "endure": [], "recovery": [],
                            "bond_on_weak_hit": [], "bond_on_strong_hit": [],
...
def _game() -> GameState:
    g = GameState(player_name="Hero", setting_genre="dark_fantasy",
                  setting_tone="serious", setting_description="A dark world.",
                  edge=1, heart=2, iron=1, shadow=1, wits=2, backstory="Was a farmer.")
...
def test_validate_narration_returns_violations():
    ...
    provider = _MockProvider(json.dumps({
        "pass": False, "violations": ["Silver lining on MISS"], "correction": "Make it worse."}))
            ...
            if json_schema and "pass" in json_schema.get("properties", {}):
                if call_count[0] <= 2:
                    return _MockResponse(json.dumps({
                        "pass": False, "violations": ["bad"], "correction": "fix it"}))
                return _MockResponse(json.dumps({"pass": True, "violations": [], "correction": ""}))
            return _MockResponse("Rewritten narration.")

    game = _game()
    _, report = validate_and_retry(
        RetryProvider(), "Bad narration.", "prompt", "MISS", game, max_retries=2)
...
def test_validate_architect_fixes_violations():
    ...
    provider = _MockProvider(json.dumps({
        "pass": False, "violations": ["magic detected"],
        "fixed_conflict": "Political conspiracy", "fixed_antagonist": "Corrupt senator"}))
...
def test_register_extracted_npcs_skips_player():
    ...
    max_id = register_extracted_npcs(game, [
...
def test_apply_world_setup_replace_vs_extend():
    ...
    apply_world_setup(game, {
        "clocks": [{"name": "New", "clock_type": "threat", "segments": 4, "filled": 1}],
        "location": "Market", "scene_context": "Busy.", "time_of_day": "midday",
    }, clocks_mode="replace")
...
def test_content_boundaries_block_injects_lines():
    _stub()
    _load_prompts()
    from straightjacket.engine.prompt_blocks import content_boundaries_block
    game = _game()
    result = content_boundaries_block(game)
    assert "spiders" in result
    assert "dog" in result


def test_content_boundaries_block_from_creation_data():
    _stub()
    _load_prompts()
    from straightjacket.engine.prompt_blocks import content_boundaries_block
    result = content_boundaries_block(creation_data={"content_lines": "no gore", "wishes": ""})
    assert "gore" in result


def test_backstory_block_injects_text():
    _stub()
    _load_prompts()
    from straightjacket.engine.prompt_blocks import backstory_block
    result = backstory_block(_game())
...
def test_narrative_direction_unknown_result_uses_default():
    _stub()
    from straightjacket.engine.prompt_blocks import narrative_direction_block
    result = narrative_direction_block(_game(), "UNKNOWN")
    assert "tempo:moderate" in result
...
def test_story_context_block_includes_conflict():
    ...
    game.narrative.story_blueprint = StoryBlueprint(
        central_conflict="Shadow rises", structure_type="3act",
        thematic_thread="Cost of survival",
        acts=[StoryAct(phase="setup", title="Gathering", goal="Find allies",
...
def test_story_context_block_epilogue_dismissed_open_ended():
    ...
    game.narrative.story_blueprint = StoryBlueprint(
        central_conflict="X", structure_type="3act", thematic_thread="Y",
        acts=[StoryAct(phase="climax", title="C", scene_range=[15, 20])],
        triggered_transitions=["act_0"], story_complete=True, possible_endings=[])
...
def test_campaign_history_block_includes_chapters():
    ...
    game = _game()
    game.campaign.campaign_history = [
        ChapterSummary(chapter=1, title="The Beginning", summary="It all started here.")]
    result = campaign_history_block(game)
    assert "The Beginning" in result


def test_get_narrator_system_assembles_all_blocks():
    _stub()
    _load_prompts()
    from straightjacket.engine.prompt_blocks import get_narrator_system
    from straightjacket.engine.models import EngineConfig
    result = get_narrator_system(EngineConfig(narration_lang="English"), _game())
...
def test_create_with_retry_retries_on_connection_error():
    ...
    resp = create_with_retry(FlakeyProvider(), max_retries=2, model="m", system="s",
...
def test_create_with_retry_raises_on_exhaustion():
    ...
    with pytest.raises(ConnectionError):
        create_with_retry(AlwaysFail(), max_retries=1, model="m", system="s",
                           messages=[{"role": "user", "content": "hi"}], max_tokens=100)


# ── strings_loader.py ────────────────────────────────────────

def test_get_string_missing_returns_key():
    from straightjacket.strings_loader import get_string
    assert get_string("nonexistent.key") == "nonexistent.key"


def test_get_string_substitutes_variables():
    from straightjacket.strings_loader import reload_strings, get_string
    reload_strings()
    result = get_string("actions.game_loaded", name="TestGame", scene=3)
...
def test_run_deferred_director_applies_guidance():
    ...
    provider = _MockProvider(json.dumps({
        "scene_summary": "Tense.", "narrator_guidance": "Build tension.",
        "npc_guidance": [], "pacing": "building", "npc_reflections": [],
        "arc_notes": "Progressing.", "act_transition": False}))
    game = _game()
