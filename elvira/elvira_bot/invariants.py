...
def assert_game_state(game: GameState, turn: int) -> list[str]:
    ...
    check(0 <= res.health <= _e.resources.health_max,
          f"health={res.health} out of [0,{_e.resources.health_max}]")
    check(0 <= res.spirit <= _e.resources.spirit_max,
          f"spirit={res.spirit} out of [0,{_e.resources.spirit_max}]")
    check(0 <= res.supply <= _e.resources.supply_max,
          f"supply={res.supply} out of [0,{_e.resources.supply_max}]")
    check(_e.momentum.floor <= res.momentum <= _e.momentum.max,
          f"momentum={res.momentum} out of [{_e.momentum.floor},{_e.momentum.max}]")
    check(res.momentum <= res.max_momentum,
          f"momentum={res.momentum} > max_momentum={res.max_momentum}")

    # Chaos factor (from engine.yaml)
    check(_e.chaos.min <= world.chaos_factor <= _e.chaos.max,
          f"chaos_factor={world.chaos_factor} out of [{_e.chaos.min},{_e.chaos.max}]")

    # Scene count
    check(game.narrative.scene_count >= 1,
          f"scene_count={game.narrative.scene_count} < 1")

    # Crisis/game_over consistency
    if res.health > 0 and res.spirit > 0:
        check(not game.crisis_mode,
              f"crisis_mode=True but health={res.health} spirit={res.spirit} (both > 0)")
    if res.health <= 0 and res.spirit <= 0:
        check(game.game_over,
              f"game_over=False but health={res.health} spirit={res.spirit} (both <= 0)")
    ...
    if game.narrative.session_log:
        ...
        check(last.scene == game.narrative.scene_count,
...
def _check_npc(npc: NpcData, turn: int, violations: list[str], _e) -> None:
    ...
    check(npc.status in ("active", "background", "deceased", "lore"),
          f"invalid status '{npc.status}'")
    check(npc.disposition in ("hostile", "distrustful", "neutral", "friendly", "loyal"),
          f"invalid disposition '{npc.disposition}'")
    check(0 <= npc.bond <= npc.bond_max,
          f"bond={npc.bond} out of [0,{npc.bond_max}]")
    ...
    for alias in npc.aliases:
        check(alias.lower() != name_lower,
              f"alias '{alias}' duplicates primary name")

    # Memory limits
    max_mem = _e.npc.max_memory_entries
    check(len(npc.memory) <= max_mem + 5,  # +5 tolerance for mid-turn state
...
def _check_clock(clock: ClockData, turn: int, violations: list[str]) -> None:
    ...
    check(clock.clock_type in ("threat", "scheme", "progress"),
          f"invalid clock_type '{clock.clock_type}'")
    check(clock.segments > 0, f"segments={clock.segments} <= 0")
    check(0 <= clock.filled <= clock.segments,
          f"filled={clock.filled} out of [0,{clock.segments}]")
    if clock.fired:
        check(clock.filled >= clock.segments,
              f"fired=True but filled={clock.filled} < segments={clock.segments}")
