...
Third dead-code pass, focused on the ~20 vulture hits in `engine_config_dataclasses.py`, `models_base.py`, `models_story.py`, and `datasworn/moves.py`. Most of vulture's complaints turned out to be false positives — dataclass attributes that vulture cannot trace through dict-assignment, getattr-resolution from a string, or test-fixture attribute access. The genuine finds clustered around one architectural drift, which is the real story of this release.

The `momentum:` and `recovery:` yaml blocks claimed to drive momentum gain on action moves and recovery healing amounts via per-tier tables (`weak_hit: 1`, `strong_hit: {standard: 2, great: 3}`). Neither was ever read. Action-move momentum gain is not a global table at all in this engine — it lives in per-move outcome text parsed at runtime. Recovery healing amounts are per-move parameters in `move_routing.yaml` (the `recovery: amount` field on the suffer/recovery handlers). The two yaml sections were leftover scaffolding from an older design. Meanwhile, the suffer-move handler in `mechanics/move_outcome.py` was hardcoding `+1` momentum on strong hits without recovery, `-1` momentum on weak-hit recovery exchanges, and the matching `"+1"` / `"-1"` consequence label strings — exactly the values the abandoned yaml had been pretending to govern. The yaml lied, the code drifted, neither side knew.

The fix names what the code actually does. `MomentumGain` is replaced by `SufferRecoveryGain` with two fields, `strong_hit_gain` (momentum awarded when track recovery is unavailable on a strong hit) and `weak_hit_exchange_cost` (momentum spent to convert a weak hit into +recovery on the affected track). `MomentumConfig` keeps `floor`, `max`, `start`, and gains `suffer_recovery: SufferRecoveryGain`; the dead `gain` and `loss` fields are gone. `RecoveryConfig` is removed entirely because the per-move `recovery:` parameter already carries the healing amount and nothing read the per-tier table. `engine/recovery.yaml` is deleted; `engine/momentum.yaml` is rewritten to the new shape. The three hardcoded sites in `apply_suffer_handler` now read `_e.momentum.suffer_recovery.strong_hit_gain` and `.weak_hit_exchange_cost`, and the consequence labels format from those same values via f-string. If the suffer-recovery numbers ever need to change, the yaml is now the place to do it.

Bycatch: `NpcConfig.max_observations` removed from both the dataclass and `engine/npc.yaml`. The field had a single mention in a stale docstring in `npc/memory.py:135` referencing a parameter that the actual consolidation algorithm replaced with `max_memory_entries` plus `consolidation_recency_ratio`. The docstring was rewritten to describe what the function does now.

Not removed, with reasons. The other ~17 vulture hits all failed the dual-side test: they were either false positives where reads happen via patterns vulture cannot follow, or they were Python-side-only runtime event fields with no yaml counterpart and no clear case for either deletion or migration. `THREAT_CATEGORIES` (a hardcoded category tuple in `models_base.py`) was considered for migration to `engine/threats.yaml` but rejected — there is no consumer that would read it, and adding a validator just to give the tuple a job would be feature work outside this pass. `antagonist_force` and `arc_notes` are heavily used by architect and director schemas, prompts, and integration tests; vulture missed all of it. `ChapterSummary.scenes` is set via `data["scenes"] = game.narrative.scene_count` and in the architect fallback constructor; vulture missed the dict-assignment. `ThreatEvent.ticks_added`, `ClockEvent.autonomous`, and `RandomEvent.meaning_table` are written when events are constructed but read by no consumer except tests in the case of `ticks_added` — they are either preparing for future consumers or were once read and the reader was removed without the field; either way, single-side removal is gambling and migration to yaml does not fit (these are runtime event fields, not configuration).

Delivery gate: 793 tests green (unchanged from 0.70.0), ruff and ruff format clean on 147 files, mypy clean on 92 source files.
