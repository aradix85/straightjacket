...
One model for everything. Fifteen Elvira sessions, eight turns each in Classic, Starforged, and Sundered Isles with the aggressor style, every role on the candidate including Elvira's player and judge; turn time and first sentence are medians, cost is per session and includes the player and judge. GPT-6 Luna: 7.7 s, 2.5 s, $0.010, no AI failure. GLM 5.3 Fast (Fireworks): 11.9 s, 4.7 s, $0.269, no AI failure, but its Brain twice filled `bonus_id` with junk and twice left track fields empty. Gemini 3.8 Flash (OpenRouter): 7.5 s, 2.6 s, $0.087, no AI failure, and four turns where the streamed text differed from the final narration. GLM 5.3 (Nebius): 18.0 s, 7.2 s, $0.189, two time-outs after 120 seconds in one session (metadata extraction and a revelation check). Qwen3.8 Max (Fireworks): 26.6 s, 6.3 s, $0.321, six empty narrations rolled back. No model chose an unavailable move. Qwen's empty narrations were traced: on real narrator prompts Fireworks ignores `reasoning_effort: none` and the template switch `enable_thinking: false` for Qwen3.8 Max, which thought 700 to 2450 words in `reasoning_content` before writing (28 to 40 seconds per narration), and an empty narration is a narration whose thinking used up the token limit; `low` thinks less (590 to 710 words, 17 to 20 seconds). The user leans towards GLM 5.3 Fast through Fireworks and wants to try Gemini 3.1 Pro through Google's own API before deciding.

Streaming a literal unicode escape. Some models now and then write a character as its escape, `\u201c` for a curly quote. `ai/provider_base.py` → `post_process_response` has always decoded those in the final narration, but the sentences streamed to the player never passed through it, so a screen reader could read out a backslash and four hex digits while the saved narration was correct. Elvira's streaming check caught it with Gemini; twelve live Gemini narrations through the engine's own adapter reproduced it twice. `ai/sentence_stream.py` → `SentenceStream.feed` now decodes complete escapes as they arrive, before sentence boundaries are found, so an escape split across chunks is decoded once its last digit arrives and an escaped closing quote does not confuse the boundary; the decoder is now public (`decode_literal_unicode_escapes`). New test: an escape split across two chunks streams as the character; it failed on the previous code.

Junk in the Brain's id fields. GLM 5.3 Fast wrote `infiltrator#0` (a real bonus) and then kept going: `{`, invisible characters, repeated dice notation, fragments of the prompt. Reproduced with the Brain prompt of a Starforged character on the Infiltrator path: with the schema enforced, 2 of 8 answers at temperature 0.5 and 3 of 8 at 0 had junk after `#0`; without an enforced schema 0 of 8; with `bonus_id` restricted to the offered bonuses 0 of 16. The fault is in how the provider's schema enforcement lets GLM close a free string, not in the model or the temperature. `ai/schemas.py` → `get_brain_output_schema` now restricts `bonus_id` to the bonuses offered, `target_npc` to the ids of the NPCs the prompt lists (active and background), and `target_track` to the names of the active tracks, which is what the prompt shows; an empty list allows only null. `ai/brain.py` builds the prompt lists and these choices from the same helpers and checks the answer against them after parsing, as it already did for moves, so a provider that does not enforce the schema fails the turn through the AI-failure path instead of storing a wrong reference. New tests: the schema offers exactly the existing bonuses, NPCs, and tracks; a bonus the game does not offer is refused while the offered one plays; the refusal test failed without the check.

Checked live, four Elvira sessions of eight turns with the aggressor style: GPT-6 Luna in Classic, and the whole game on GLM 5.3 Fast (Fireworks) in Classic, Starforged, and Sundered Isles. No unknown bonus, no refused choice, no AI failure; the Brain still twice left a new track's name and rank empty in Starforged, which the engine fills in as before.

Gemini 3.1 Pro through OpenRouter was probed as a narrator: reasoning is mandatory, and at minimal, at low, and with a 128-token reasoning budget it still thought 1258 to 1920 tokens before the first word (14.5 to 19.5 seconds), so it is too slow for any role here. The user's Google billing setup did not complete, so Google's own API was not tried.

Quality gate: 1463 tests green, twenty-nine project-rule scans clean, coverage 90.10%, ruff check and ruff format clean, mypy --strict clean on 109 source files. Save format unchanged.

## [2026.09.24.57] — 2026-09-25

A rubric that values atmosphere, a two-family judging panel, fifteen narrator models compared, Nebius as a provider, and Elvira on one model for everything, all at the user's request.

The rubric. The user wants rich, atmospheric prose and sees sensible invention as a strength. The old `restraint` criterion penalised every invented name or detail and there was no criterion for mood, so the rubric rewarded sparse prose. `tests/modeltest/modeltest_prompts.yaml` now welcomes invention that enriches a scene and penalises only invention that contradicts the game state or overrides the outcome, and adds `atmosphere` (sense of place, mood, genre). The criteria live in `modeltest_config.yaml` (`criteria`) and the judge schema is built from them (`measure.py` → `judge_schema`), so the harness no longer borrows Elvira's.

The panel. Blind judging hides model names, not styles: Claude Sonnet 5 favoured Claude prose, and GPT-6 Sol, which did not favour itself over Luna, rated both OpenAI models highest. The harness now judges with GPT-6 Sol and Grok 4.7 (xAI, no contestant of its own, eighth on EQ-Bench's creative-writing board as a writer, $1.60 and $4.80 per million tokens), and the report lists each judge's overall score per model. New options: `--rejudge <run files>` scores stored narrations again with the current rubric and judges, and `--report <run files>` merges runs into one report without an API call. New test: rejudging replaces every verdict and leaves the input untouched.

Results, twenty scenes and five narrations each, with the new rubric and panel; overall, result integrity on a miss, atmosphere, first text, whole narration, dollars per 100 narrations, and the scene-by-scene comparison with GPT-6 Luna. Better: Gemini 3.8 Flash 7.63, 4.86, 4.84, 1.2 s, 3.4 s, $0.31; GLM 5.3 Fast (Fireworks) 7.62, 4.58, 4.95, 0.9 s, 4.0 s, $0.79; GLM 5.3 Prime 7.56, 4.44, 4.91, 1.4 s, 5.5 s, $1.04; Claude Sonnet 5 7.44, 4.14, 4.78, 3.9 s, 12.1 s, $1.49. Not distinguishable: Qwen3.8 Max 7.49 and Qwen3.8 2.4T 7.41 (Fireworks, reasoning off; 40 and 36 of 100 answers came back empty, and the first text took a median of over 6 seconds), GLM 5.3 through Nebius 7.45, 4.20, 4.89, 1.2 s, 5.3 s, $0.52, GLM 5.2 Fast 7.30, GPT-6 Luna 7.23, 4.20, 4.12, 0.8 s, 2.8 s, $0.03, GPT-6 Sol 7.15, GLM 5.3 Flash 7.15, Kimi K3 Fast 7.04. Worse: Ember-1 6.92, Qwen3.5 397B (Nebius) 6.63, Nemotron 3 Ultra (Nebius) 6.53 with result integrity 2.78. Grok rated Luna 6.89 and Sol 6.72, the lowest of all, where Sol rated them 7.57 and 7.58. The same GLM 5.3 scored about 0.17 lower through Nebius than through Fireworks by both judges, consistent with Nebius serving it in NVFP4.

Providers. Qwen3.8 Max can switch reasoning off only through Fireworks ($2.00, $0.25 cached, $6.00); through Alibaba on OpenRouter reasoning is mandatory and at minimal it spent 2000 tokens thinking without a word of narration after 56 seconds, and the open 2.4T variant behaves the same with OpenRouter's other providers. Nebius Token Factory joins the harness as a provider (`https://api.tokenfactory.nebius.com/v1`, `NEBIUS_API_KEY`); the user's old key had expired and `NEBIUS_BASE_URL` pointed at the old address. There GLM 5.3 at reasoning effort low wrote about 150 tokens per second, as fast as Fireworks' Fast tier at two thirds of its price; at effort none Nebius does not switch thinking off but moves it into the narration. Kimi K3 without reasoning matched Fireworks' K3 Fast at the standard price; MiniMax M3 cannot stop reasoning; Nebius has no Qwen 3.8.

Elvira on one model. The user wants one model for every role, including Elvira's own player and judge, which also keeps prompt tuning simple. Her judge now uses the Brain role's model through the session's provider, and `judge` in `tests/elvira/elvira_config.yaml` keeps only its switches, token limit, and reasoning setting. `--model <name>` puts every call of a session on a contestant from the harness configuration through `ModelSwap` in `tests/elvira/elvira_bot/narrator_swap.py`; `--narrator` remains and swaps the narrator alone. A judge from the model under test favours that model, so Elvira's scores now compare versions of one model, while comparisons between models stay with the harness panel. New test: every role moves to the chosen model.

Checked live: a three-turn Elvira session in Classic with GPT-6 Luna playing, narrating, and judging scored every audited turn, with no problems and no judge errors.

Quality gate: 1460 tests green, twenty-nine project-rule scans clean, coverage 90.08%, ruff check and ruff format clean, mypy --strict clean on 109 source files. Save format unchanged.

## [2026.09.24.56] — 2026-09-25

A ten-model narrator comparison with reasoning off or as low as each model allows, a judge that favours itself found and removed, and GPT-6 Sol as the judge for both Elvira and the harness.

Speed first. Probes on the heaviest scene showed that reasoning can be switched off only for GPT-6 Luna, GPT-6 Sol, Kimi K3, Kimi K3 Fast, Ember-1, and GLM 5.2 Fast; Gemini 3.8 Flash, GLM 5.3 Fast, Prime, Flash, and FlashX refuse it and run at `minimal` or `low` (Gemini then used no reasoning tokens at all); Claude Opus 5.5 accepts only adaptive thinking and at effort low still took 4.6 to 7 seconds to its first word, at $4 and $20 per million tokens against $2 and $10 for GPT-6 Sol and Claude Sonnet 5. Fireworks' Fast tiers are routers (`accounts/fireworks/routers/...`), cost half as much again as the standard tier, and are really faster: Kimi K3 Fast wrote 90 to 105 tokens per second and GLM 5.3 Fast 140 to 155, against 55 to 72 for the fastest route OpenRouter chose for the same models (Wafer and Together) and 36 to 53 for Fireworks' standard tier. First text came after about a second everywhere.

The comparison: twenty scenes, five narrations each, ten models. Claude Sonnet 5 stopped judging after 1432 of 2000 verdicts, when the Anthropic credit ran out (400: credit balance too low), so six models had one judge only. Where both judges scored, Sonnet rated its own narration 0.85 above GPT-6 Luna while GPT-6 Sol rated it 0.22 below; Sol rated itself 0.10 below Luna by both judges' account. The results below therefore use GPT-6 Sol alone, which scored all 1000 narrations; overall out of 10 with 95% interval, result integrity on a miss, first text, whole narration, words, dollars per 100 narrations, and the scene-by-scene comparison with Luna. Gemini 3.8 Flash 7.34 (7.11 to 7.55), 4.76, 1.2 s, 3.4 s, 252, $0.31, equal. GPT-6 Luna 7.32 (7.11 to 7.54), 4.00, 0.8 s, 2.8 s, 136, $0.03. GPT-6 Sol 7.22, 4.32, 1.2 s, 6.4 s, 154, $0.68, equal. Claude Sonnet 5 7.10, 3.88, 3.9 s, 12.1 s, 283, $1.49, equal. GLM 5.3 Fast 7.09, 4.28, 0.9 s, 4.0 s, 292, $0.79, equal. GLM 5.3 Prime 7.06, 4.12, 1.4 s, 5.5 s, 289, $1.04, equal. GLM 5.2 Fast 6.89, 3.48, worse. Kimi K3 Fast 6.65, 4.16, worse. Ember-1 6.65, 3.52, worse. GLM 5.3 Flash 6.62, 3.36, worse. No model narrates measurably better than Luna; Gemini 3.8 Flash matches her and is clearly more honest on a miss, at ten times the price and nearly twice the words. Kimi K3, better than Luna at reasoning low in 2026.09.24.54, is worse with reasoning off, so switching reasoning off can cost quality.

Consequences. Elvira's judge is GPT-6 Sol (`tests/elvira/elvira_config.yaml`); the harness judges with GPT-6 Sol alone (`tests/modeltest/modeltest_config.yaml`), and its test counts judges from the config instead of assuming two. The new baseline `tests/modeltest/baseline_luna_gemini_sol.json` holds this run's GPT-6 Luna and Gemini 3.8 Flash narrations with Sol's verdicts, the reference for tuning Luna's narrator prompt; `baseline_gpt-6-luna.json` is gone. The harness configuration gains the ten contestants with their reasoning settings, and OpenRouter contestants route by latency. The user chose to tune the narrator prompt for GPT-6 Luna next and compare again with Gemini 3.8 Flash. ARCHITECTURE.md and the roadmap name the new judge and baseline, and now say twenty scenes where they still said ten.

Checked live: a three-turn Elvira session in Sundered Isles with GPT-6 Sol as judge scored every audited turn and reported no problems and no judge errors.

Quality gate: 1458 tests green, twenty-nine project-rule scans clean, coverage 90.05%, ruff check and ruff format clean, mypy --strict clean on 109 source files. Save format unchanged.

## [2026.09.24.55] — 2026-09-25

Three engine bugs found by the narrator sessions of 2026.09.24.54 fixed, at the user's request, and three small fixes in the test tools.

A bond keyed-scene for a character who is not an NPC. A random event with an NPC focus picks its target from the characters list, which by Mythic's rules also holds people who were only named in the narration; `game/scene_finalization.py` gives those ids like `char_1`. `mechanics/random_events.py` → `spawn_keyed_scene_from_random_event` still spawned a bond-threshold keyed scene for such a target, and the first evaluation raised in `keyed_scenes.py` (unknown npc_id), which ended an Elvira session after six turns. A bond needs an NPC, so the spawner now spawns the scene only when the target is one, and logs why it skips otherwise; the event itself is unchanged, and the strict evaluation stays, since an unknown NPC there would now mean broken data. New test: a random event aimed at `char_1` spawns no keyed scene; it failed on the previous code.

The Brain could play a move the situation does not allow. `ai/brain.py` listed only the available moves in the prompt, but `ai/schemas.py` → `get_brain_output_schema` allowed every move of every setting, so in one session the Brain played Strike with no fight open: the progress mark found no combat track and was skipped, and the combat position was set anyway, which Elvira's combat-sync invariant caught on three turns. The same gap allowed moves such as Fulfill Your Vow without a vow. The schema now takes the moves that are available now (plus the engine moves such as dialog) and is built per call instead of cached, so a provider that enforces the schema cannot choose an unavailable move; and `call_brain` checks the chosen move against the same list, so a provider that does not enforce it fails the turn cleanly through the existing AI-failure path instead of corrupting the state. New tests: a strike is offered only with a fight open; a Brain that answers with an unavailable move raises, which failed without the check; an available move plays; and the per-call Brain schema passes the strictness check for OpenAI.

Unknown tool arguments cost a Director round. The tool definitions forbid extra properties, but without strict function calling a model can still send them, and the Director sent `threads` to `query_active_threads`; the call failed and the round was wasted. `tools/handler.py` → `execute_tool_call` now drops arguments the handler does not take, runs the tool with the rest, logs a warning, and adds `ignored_arguments` to the result so the model learns. Strict function calling was not chosen: it needs every parameter required and optional ones nullable, which changes every tool's meaning, and it only helps with OpenAI. New test; it failed on the previous code.

Test tools. Elvira's spatial check compared location names literally and counted a reworded place (a quay in a harbor whose name is unknown, then a quay in an unnamed harbor) as a move; it now compares content words and treats two descriptions as the same place when most words of the shorter one appear in the other, and a new test keeps real moves distinct. The model-comparison harness keeps error messages whole instead of cutting them at 200 characters, which had cut the link OpenRouter gave for its age confirmation. Elvira's banner names the narrator chosen with `--narrator` instead of the configured one.

Elvira, two live runs on GPT-6 Luna with Claude Sonnet 5 as judge. Eight turns in Classic as explorer: every Brain move was available (among them `combat/battle`, Ironsworn's one-roll fight, which needs no open fight), the injected outage on turn 3 rolled back cleanly, and the one finding was the narrator inventing a dialogue the player never started. The combat scenario in Starforged: the fight ran through four combat moves (Strike and Clash), the combat-sync invariant never fired, and no move was refused; its two findings were narration ones as well (a player action nobody chose, and a dropped weapon on a clean strong hit). No Brain answer was refused as unavailable in either run, and no tool call carried unknown arguments.

Quality gate: 1458 tests green, twenty-nine project-rule scans clean, coverage 90.09%, ruff check and ruff format clean, mypy --strict clean on 109 source files. Save format unchanged.
...
Scenarios. Some situations hardly ever occur in a normal run, however long: game over and succession, a chapter's end, a momentum burn, a fight, a filling clock. `--scenario <name>` prepares one after character creation and reports whether the run reached what it expects; `--scenario all` plays them in turn. The scenarios live in `tests/elvira/elvira_config.yaml`, each with its turns, optional style, burn setting, chapter count, a fixed player directive, and the coverage targets it expects; `tests/elvira/elvira_bot/scenarios.py` prepares the state and refuses an expected target Elvira does not know. `near_death` sets health and spirit to 0, so the engine's own crisis check ends the game on the first action turn and succession follows; `chapter_end` marks the acts as entered and puts the scene count one short of the final act's end; `momentum_burn` sets momentum to its maximum and takes every burn; `combat` opens a combat track in control and tells the player to attack; `clock` puts the first open clock one segment from full. The report gains a Scenario section with what was prepared, reached, and not reached. `_play_turn` now takes its directive from the caller, and the scenario plumbing sits in small helpers so the run loop stays under the complexity ceiling.

Found by the first scenario run: the combat scenario first told the player to take physical risks, the directive Elvira's coverage steering also used for combat. The player sprinted, chased, and vaulted for six turns, the Brain rightly read every turn as Face Danger, and no combat move was ever played, which explains why combat never appeared in any Elvira run. A new directive, `bot_turn_directive_attack` in `tests/elvira/elvira_prompts.yaml`, tells the player to strike at an enemy; the combat scenario and the coverage steering for combat both use it. The same run showed `near_death` with health 0 and spirit 1 ending without game over: game over needs both at 0, checked after an action turn, and eight turns brought only one miss, so the scenario now starts with both at 0.

The second run, with both fixes, reached every scenario's targets: `near_death` ended in game over and a full succession (predecessor archived, inheritance rolls, a new character who then also fought and matched), `chapter_end` wrote the epilogue and opened chapter 2 on a new kishōtenketsu blueprint, `momentum_burn` offered and took a burn, `combat` played combat moves, and `clock` filled its clock and fired it. One problem in that run came from the scenario itself: `near_death`'s first turn was a dialog, the crisis check runs after action turns only, and Elvira's invariant rightly flagged health and spirit at 0 without game over. Real play cannot reach that state, since 0 and 0 only arise from an action's consequences; the scenario now tells the player to take physical risks so the first turn is an action, and a third run confirmed it: game over and succession, and the invariant no longer fires. The other findings are real and join the roadmap's Elvira list: NPCs moving between locations without the narration showing it, narration deciding the player character's sensations, the Director reaching its tool-round limit, and a new one, the Director calling `query_active_threads` with an argument the tool does not take.

Model-comparison harness, ready for the next measurement. It now measures the time to the first text through `stream_with_retry`, the path the game itself streams through, besides the time for the whole narration, and reports price per 100 narrations apart from the judging cost, which is the same for every contestant. It can declare providers of its own in `modeltest_config.yaml` (Fireworks, OpenRouter), built with `build_adapter`, which `ai/api_client.py` now exposes instead of the private `_build_adapter`; the engine's `config.yaml` keeps its two providers. The contestants are the five chosen from the public writing leaderboards with the user: GLM 5.3, Muse Spark 1.3 Contributor, Qwen3.8 Flash, Kimi K3, and Gemini 3.8 Flash, all through OpenRouter with reasoning effort low.

First measurement with it: ten scenes, three attempts each, judged by GPT-6 Sol and Claude Sonnet 5. Per model: overall out of 10, result integrity on a miss out of 5, first text, whole narration, words, and price per 100 narrations at OpenRouter's list prices (its routing can be cheaper). GPT-6 Luna, as configured: 7.00, 4.08, 0.9 s, 3.1 s, 137 words, $0.03, in line with the baseline's 6.98. GLM 5.3: 7.05, 4.42, 1.2 s, 6.4 s, 290 words, $0.52. Gemini 3.8 Flash: 7.22, 4.58, 1.9 s, 5.2 s, 240 words, $0.30. Kimi K3: 7.53, 4.33, 4.3 s, 13.7 s, 239 words, $1.31. Qwen3.8 Flash: 6.86, 4.00, first text after 54 to 89 seconds where measured and 77.5 s for the whole narration, too slow to play with at reasoning effort low, $0.29. Muse Spark 1.3 Contributor: all 30 calls refused with 403, because OpenRouter requires an 18+ age confirmation on the account before this model can be used. The three stronger contestants write about twice Luna's length, which a screen-reader player hears as longer turns; with thirty narrations per model, differences of a few tenths may be chance.

Schema strictness as a test. The old harness's `check_openai_schemas.py` checked that every object in every engine schema requires all its properties, which OpenAI's strict structured outputs demand, and then made a live call. The static part is now `tests/test_schema_strictness.py`, with a second test that proves the check catches a loose property; the live part is covered by every Elvira run. The folder `Documents\sj_modeltest` went to the recycle bin at the user's request: its scenes, scripts, and old baseline are replaced by `tests/modeltest/`, its remaining provider probes were one-off or are covered by `tests/test_providers.py` and the startup model check, and its raw 24 September results are history whose conclusions stand in this CHANGELOG.

Tests: `tests/test_elvira_scenarios.py` (every configured scenario names known targets, an unknown target is refused, momentum set to its maximum, the story completes on the scene after `chapter_end`, `combat` opens a track and a position, `clock` leaves one segment); `tests/test_schema_strictness.py`.

Quality gate: 1438 tests green, twenty-nine project-rule scans clean, coverage 90.08%, ruff check and ruff format clean, mypy --strict clean on 109 source files. Elvira: seven scenario runs across Classic, Starforged, and Sundered Isles, reported above. Save format unchanged.
...
AI failure pauses the turn (decision 3). The design document says that when an AI call fails or returns unusable output, the engine changes no game state, the game pauses, and the player gets a clean message. Until now the engine did the opposite, and more quietly than ARCHITECTURE.md said: a turn whose narrator call failed after its retries stopped halfway with the roll and its consequences already applied to the session's game, and the player heard the raw exception text; a failed Brain call became a dialog turn; a failed revelation check counted as confirmed; a persistent refusal returned an empty answer, and an empty narration was replaced by a fixed English line in `parser.py`, "(The narrator pauses, gathering thoughts...)", so a failure sounded like a narrator pausing. Now `ai/provider_base.py` has `AIUnavailableError`, raised by `create_with_retry` after the last retry and on a persistent refusal, by `call_narrator` on an empty answer, by `narrate_scene` when parsing leaves no prose, and by `call_brain` on any failure. The handlers for a turn, a correction, and a momentum burn take a snapshot before the call; on any failure they restore it, restore the previous `last_turn_snapshot`, resync the database, and tell the player in plain words that the narrator could not be reached (or that something went wrong in the engine) and that nothing in the story changed (`error.ai_unavailable`, `error.unexpected`, `error.nothing_changed` in `strings/error.yaml`). The other handlers no longer send raw exception text. `web/server.py` → `_dispatch_one_message` reports an unexpected error in a handler without its own catch instead of letting it close the connection. A failed revelation check now counts as not yet confirmed. The sixth divergence from the design document, recorded in 2026.09.24.45, is gone because the engine now follows the document here.

Loading returns exactly what was saved (decision 4). `load_game` normalised NPC dispositions, filtered aliases, recomputed `needs_reflection`, and sanitised names after loading. All four came with the first commit (0.31.0), and every path that writes NPCs already does the same cleaning, so at load time they only repaired old or dirty saves, which the strict loader of 2026.09.24.45 rejects anyway; the recomputed reflection flag could even make a loaded game behave differently from one played on. They are gone.

Step 9's shape (decision 1). The generator returns one of two typed results, `GeneratedEntity` for a new entity and `ResolvedFact` for an answered fact question, and categories with their oracle paths are registered in yaml as a mapping keyed by category, so a new category needs no Python. Recorded in roadmap step 9 and its Definition of Done.

The Yaml content boundary covers setting yaml (decision 2). Step 17 no longer plans hand-written lists of names, descriptors, and descriptions; variety comes from the Datasworn oracles and Mythic's element tables through steps 9, 10, 33, and 34. The one exception is a setting's existing `vocabulary` block, which the design document assigns to the setting as a constraint on the narrator's word choice; new entries follow measured drift only. Recorded in ARCHITECTURE.md and roadmap step 17. The roadmap's list of open architectural questions is empty again.

Correction to 2026.09.24.45: its AUDIT.md note said a handler exception always ended the WebSocket connection. Most handlers already caught their exceptions; the real faults were the half-applied state and the raw error text, both fixed here. The note is corrected.

Tests: new `tests/test_ai_unavailable.py` (a Brain failure raises instead of becoming dialog, a failed revelation check counts as not confirmed, an empty narration raises, a failed turn is rolled back and reported with the player's input removed from the chat, an unexpected handler error is reported without closing the connection). Four tests asserted the old behaviour and are rewritten: exhausted retries and a persistent refusal now raise `AIUnavailableError`, an empty epilogue raises, and prose-less narration parses to an empty string. `test_load_normalizes_npc_dispositions` becomes `test_load_returns_exactly_what_was_saved`.

Elvira, eight turns in Starforged on GPT-6 Luna: no engine warnings or errors; both save round trips went through the exact load without a difference; one NPC introduced, two Pay the Price rolls, one correction, six Director runs with 49 tool rounds; seven streamed turns, all identical to the final text. No AI call failed, so the rollback itself ran only in the tests. Two problems, both known narrator weaknesses under priority 3 and not caused by this release: a strong hit given an extra complication (turn 3) and a miss given a useful clue (turn 6). Cost about two cents.

Quality gate: 1417 tests green, twenty-nine project-rule scans clean, coverage 89.93%, ruff check and ruff format clean, mypy --strict clean on 109 source files. Save format unchanged since 2026.09.24.45.
...
Strict loading. `serialization.py` → `deserialize` skipped fields a class did not know and let dataclass defaults fill fields a save lacked: ignore-unknown-fields and default-on-old-fields, both ruled out under "No backwards compatibility", and what let the save-compatibility defaults of 2026.09.24.17, .23, .25, and .33 keep older saves loading. It now raises on both, naming the class and the fields. Gone with it: `RollResult.d2`, kept only so old saves would load (.17); the duplicate-id repair on load (.38, .39), a migration layer for saves damaged by a bug that .38 already prevents; the `.get` default on `chat_messages`; and `MemoryEntry`'s own `to_dict`, which dropped an empty `_score_debug` and relied on the default when loading. `register_extracted_npcs` built each new NPC from the opening-setup dict through `from_dict`; it now calls the constructor with the six fields the schema always delivers, and two pops of the `bond` fields removed in 0.46.0 are gone. The defaults on `next_move_bonus`, `completions`, and `asset_abilities` stay as starting values for new objects; they no longer make old saves load.

A save that no longer loads is reported instead of dropping the connection. `persistence.py` → `load_game` raises `IncompatibleSaveError`; the select-player and load handlers catch exactly that, log it, and tell the player the save comes from an older version (`actions.save_incompatible` in `strings/actions.yaml`). Without this, choosing a player with an old autosave would have ended the WebSocket connection silently, since the message loop closes on any handler exception.

Typed config. `engine/pay_the_price.yaml` and `engine/roll_bonuses.yaml` are fixed-field blocks read through `get_raw`, which "Yaml access" reserves for yaml whose keys are domain data. They now bind to `PayThePriceConfig` and `RollBonusesConfig`.

Found by the strict loader in the tests: the mock Brain answers in three test files lacked four fields the real schema always delivers, and once the loader refused them the Brain's carve-out quietly turned those turns into dialog; they now carry all thirteen. Hand-built partial dicts for WorldState, StoryBlueprint, and opening-setup NPCs take the shape production produces; six positional `RollResult` calls lose the second die; the two tests asserting that old saves still load now assert that a missing or unknown field refuses to load; the two tests of the duplicate-id repair are removed; a new handler test checks that an incompatible autosave leads to the creation screen with the message.

Documentation. ARCHITECTURE.md describes the fate system as it is: implemented and tested, but without a caller in play since 0.46.50, when its only route in, a Brain tool, went with the move to prompt injection. Known Limitations replaces "No asset mechanics" with what the roll bonuses of .33 and .41 do and what is missing, and notes that the metadata extraction lets narration establish NPCs, renames, and deaths. A sixth divergence from the design document records that the engine plays on after an AI failure where the document pauses. The Yaml access rule no longer asks for a comment at each `get_raw` call, which the no-comments rule forbids; Contributing formats `tests/` too, as the gate checks; the Testing section names what the drift scans cover. README no longer says factions move independently, names the Adventure Crafter, and says Mythic's fate questions are not yet connected. SECURITY.md drops the Cerebras example. roadmap.md: step 9 is the first priority, at the user's request, because fate is a core part of the design; the in-scope and post-flight rules no longer check for policy markers that 2026.04.26.2 abolished; step 28.2 no longer describes the cursed die as disadvantage, which the Datasworn texts contradict; step 13b's counts are marked for a recount; two open architectural questions are recorded (step 9's return shape and category registration, and whether the Yaml content boundary covers setting yaml, which step 17 depends on). AUDIT.md gains notes on the load-time corrections that remain, the orphan scan counting re-exports and tests as consumers, and handler exceptions that still drop the connection.

Elvira, eight turns in Starforged on GPT-6 Luna: no engine warnings or errors; both save round trips went through the strict loader without a difference; the opening setup registered its NPCs through the new constructor path; three Pay the Price rolls, one correction, and seven Director runs with 39 tool rounds and no fallback; seven streamed turns, all identical to the final text, first sentence after 2.9 seconds (median). One problem, not caused by this release: on turn 5 the narrator gave a miss a silver lining (audit 4 out of 10), which is priority 3 in the roadmap. Cost about one cent.

Quality gate: 1412 tests green, twenty-nine project-rule scans clean, coverage 89.85%, ruff check and ruff format clean, mypy --strict clean on 109 source files. Save format breaks: saves from before this release no longer load and are reported to the player as incompatible.
