...
Generieke patroon-grammatica voor de drie patroon-supporting trigger-types. `bond_threshold` accepteert `<npc_id>:N`, `any:N`, of `disposition_<value>:N` (gevalideerd tegen `engine/enums.yaml::dispositions`). `threat_menace_phase` en `clock_fills` accepteren `<entity_name>:N` of `any:N`. Per trigger-type een `pattern_grammar`-blok in `engine/keyed_scenes.yaml` met geldige prefixes plus matcher_strategy (`highest_bond` / `highest_menace` / `highest_filled`). Op fire-moment resolveert `evaluate_keyed_scenes` patroon-triggers naar concrete entities, zet `KeyedScene.bound_entity_id`, en respecteert "niet-eerder-gebonden" — een NPC of threat of klok die al aan een keyed-scene gebonden is wordt overgeslagen door zustermatchers. Een eenmaal gebonden patroon-scene gedraagt zich daarna als concrete-trigger op zijn entity (re-evaluatie checkt of die specifieke entity de drempel haalt, niet de hele pool opnieuw).

KeyedScene gewijzigd: nieuw `bound_entity_id: str | None = None` plus `source: str = ""` veld. Save format breekt — bestaande saves verliezen hun keyed_scenes-lijst-shape, nieuwe saves dragen de twee velden. Geen migratie. `chaos_extreme` en `scene_count` zijn entity-loos en hebben geen patroon-vorm. ARCHITECTURE.md drop "spawning is not yet wired" qualifier en gaat naar de drie-spawner-realiteit; AC-paragraaf wordt "fully wired into engine-resolved fiction". 1224 tests groen (was 1188, +36); ruff/format/mypy clean. Twee nieuwe roadmap-stappen ("Threat creation from random events and AC plot-points" en "Clock expansion" — vol-consequenties plus klok-creatie van random events plus klok-creatie van AC plot-points) ingepland tussen 7c en 13b.

Niet gedaan in deze stap: faction-filter op bond_threshold-grammatica (wacht op step 14a `FactionData`); threat-creation vanuit AC-plot-points of random events (eigen volgende stap); klok-creatie vanuit move-outcomes of random events of AC-plot-points (vangt mee in de Clock-expansion-stap); klok-vol-default-consequenties wanneer geen keyed-scene gekoppeld is (idem Clock-expansion-stap).

## [2026.05.06.2] — 2026-05-06

Stap 7b afgerond. AC character-crafting-tabellen ship als pure helpers in `mechanics/adventure_crafter.py`: `roll_character_traits(rng) -> CharacterTraits` plus drie per-tabel-lookup-helpers (`lookup_character_special_trait`, `lookup_character_identity`, `lookup_character_descriptor`). `CharacterTraits` is een frozen dataclass met `(special_trait: str, identities: list[str], descriptors: list[str])` — list-lengte 1-of-2 is de discriminator op zowel identities als descriptors. Reroll-bij-flag-fire-conventie: rol 1-33 op identity en rol 1-21 op descriptors triggeren twee directe rolls op de niet-flag-range (`rng.randint(34, 100)` / `rng.randint(22, 100)`), geen recursie of rejection-sampling-loop. Geen consumer gewired in deze stap — stap 11 (NPC tiers) consumeert `CharacterTraits` in NPC-data-flow; orphan-symbol-carve-out voor de vijf nieuwe public-symbols staat in `tests/test_project_rules.py` en wordt door stap 11 verwijderd zodra de consumer landt.

Drie spec-drift-fixes in dezelfde commit. Ten eerste: roadmap-substep 7b.1 schreef `(special_trait, identity, dual_identity, descriptors)` voor — single identity met aparte boolean-flag. Het step-4-precedent (`is_keyed: bool` weggehaald omdat `scene_type == "keyed"` overal de discriminator was) geldt symmetrisch hier: list-lengte als discriminator, geen parallelle bool. Eindshape `(special_trait, identities, descriptors)`, beide lijst-velden 1-of-2 lang. Ten tweede: substep 7b.4 schreef "result struct serialises through SerializableMixin round-trip" voor, maar `CharacterTraits` is een runtime-tijdelijk seed-object op dezelfde shape als `BlueprintSeed`/`PlotPointHit`/`TurningPoint`/`ActSeed` — geen van die heeft de mixin omdat ze niet gepersisteerd worden. Ze produceren mutaties op `narrative.*_list` velden die wel via SerializableMixin via `NarrativeState` lopen. Stap 11 zal hetzelfde patroon volgen: rolled traits voeden NPC-data-constructie, gepersisteerde staat is de resulterende `NpcData`, niet het traits-object. Serialisatie-test geschrapt. Ten derde: `data/adventure_crafter.json::character_identity` had geen entry op rol 99 — de transcriptie van AC v2 pagina 117 (Collected Tables) reproduceerde getrouw het streepje dat het boek daar zelf laat staan, vermoedelijk Tana Pigeon's eigen typo. Gat gevuld met `Spy` in deze commit, categorisch consistent met buurentries (Soldier 82, Law Enforcement 83, Scientist 84). Coverage 1..100 nu compleet.

Tests: 43 nieuwe in `tests/test_character_crafting.py` (boundary-tests per tabel, full-coverage-1..100, flag-range-detectie, dual-identity / two-descriptors paden via stub-RNG, fixed-seed-determinisme, frozen-eigenschap). ARCHITECTURE.md: AC primitives module-ownership-row uitgebreid met de vier nieuwe symbol-namen, AC primitives Key Design Decision-paragraaf uitgebreid met één zin over character-crafting-helpers plus de orphan-carve-out-status. Quality gate: 1188 tests groen (was 1145, +43 character-crafting), ruff check + format clean over 171 files, mypy zonder issues over 102 source files.

---



Stap 8 afgerond. `ai/architect.py` gesplitst in `ai/recap.py` (`call_recap`) en `ai/chapter_summary.py` (`call_chapter_summary`). Bestand was na stap 7a niet meer over architect — `call_story_architect` is daar verdwenen — en de naam dekte de inhoud niet meer. `engine/architect.yaml` hernoemd naar `engine/recap_limits.yaml`; top-level block `architect_limits:` → `recap_limits:`; veld `architect_campaign_window` → `recap_campaign_window`. Dataclass `ArchitectLimitsConfig` → `RecapLimitsConfig`. Eén stale reader in `ai/blueprint_voicing.py` (van stap 7a) bijgewerkt. Tests gesplitst: `test_architect.py` weg, `test_recap.py` nieuw met de vijf recap-tests, `test_chapter_summary.py` aangevuld met de vier chapter-summary-tests. Carve-out-lijst in `test_project_rules.py` bijgewerkt. Quality gate: 1145 tests groen, ruff check + format clean, mypy zonder issues over 102 files.

---

## [2026.05.06.0] — 2026-05-06

Stap 7a afgerond. AI-architect vervangen door AC-blueprint plus voicing-call. Plot-shape (acts, phases, turning-points, plot-beat seeds, ending seeds) komt uit Adventure Crafter; één kleine AI-call (`call_blueprint_voicing`) vertaalt die seeds naar setting-prosa voor velden die tafels niet kunnen leveren (central_conflict, antagonist_force, thematic_thread, per-act goal/mood/title, revelation content, ending type/description). De AI kiest geen plot-beats meer — hij geeft ze setting-stem.

Pipeline: `assemble_blueprint_seed_from_ac` (3-act-pad, rolt turning-points en assembleert revelation-seeds plus ending-seeds uit `narrative.plotlines_list`) of `assemble_blueprint_seed_kishotenketsu` (kishotenketsu-pad, geen turning-points, vier phase-acts uit yaml) produceert een frozen `BlueprintSeed`. Daarna `call_blueprint_voicing` → dict-or-None. `materialize_blueprint(seed, voicing)` zet beide samen tot een complete `StoryBlueprint`; bij voicing-faal blijft `narrative.story_blueprint` op None en valt de Director terug op zijn no-blueprint-defaults (graceful degradation, geen session-crash). Scene-ranges per act komen mechanisch uit `scene_range_default` evenly-split; geen verzonnen verdeling. `act.phase` per act komt letterlijk uit yaml-phase-lijst.

Verwijderd: `call_story_architect`, `_clean_act_moods`, `_validate_scene_ranges`, `_build_architect_user_msg`, `prompts/architect.yaml`, schema `get_story_architect_output_schema`, schema-titel `story_architect_output`, role-cluster-mapping voor `architect`, yaml-block `architect:` met `forbidden_moods`, ai_text-veld `default_act_mood`. Toegevoegd: `prompts/blueprint_voicing.yaml`, `prompts/chapter_summary.yaml` (chapter_summary uit oude architect.yaml gehaald), `engine/adventure_crafter.yaml` `blueprint:`-sectie met phase-namen plus aantallen (geen content), schema `get_blueprint_voicing_schema`, schema-titel `blueprint_voicing_output`, role-cluster-mapping voor `blueprint_voicing: creative`. `BlueprintConfig`-dataclass nieuw. `ArchitectConfig`-dataclass weg.

Dataclass-defaults op `StoryAct`, `Revelation`, `PossibleEnding` weggehaald (alle velden nu required); `StoryBlueprint`-scalar-defaults weg, empty-collection-factories blijven, runtime-vlag `story_complete: bool = False` blijft (initiële runtime-staat, geen config-veld). Save format breekt zonder migratie. Drie callsites herwerkt: `game/game_start.py`, `game/chapters.py`, `game/succession.py` — `_apply_blueprint`-signature nu `(game, seed, voicing)` in plaats van `(game, provider, blueprint_dict)`.

Tests: `test_architect.py` (24 oude tests) terug naar 9 die nog gelden (rest verwijderd met de functies). Nieuwe `test_blueprint.py` met 16 tests over seed-assembly, materialize, kishotenketsu-pad, frozen-dataclass-eigenschap, plus grep-test dat `call_story_architect` nergens meer in src/. Vier andere test-files bijgewerkt voor strictere dataclass-callsites (`test_director.py`, `test_models.py`, `test_story_state.py`, `test_web.py`, `test_prompt_boundary.py`, `test_chapters.py`). `_helpers.py` kreeg `make_act`, `make_revelation`, `make_ending`, `make_blueprint`. Quality gate: 1145 tests groen, ruff check + format clean, mypy zonder issues over 101 files.

ARCHITECTURE.md bijgewerkt: module-ownership-tabel rijen voor story-structure en chapter-transition; AI-cluster-tabel `architect` → `blueprint_voicing`; file-map `ai/architect.py` opgesplitst (technisch in stap 8, doc al klaar voor de samengevoegde delivery); carve-out-paragraaf bijgewerkt; "AC's role expands"-paragraaf herschreven met blueprint-pad plus expliciete bounded-scope (geen mid-chapter-AC-scheduling, dat blijft bij de zes bestaande pacing-systemen); de Known-Limitation "Blueprint is AI-generated at game start" weggehaald omdat hij niet meer geldt.

Eén ontwerp-fout vroeg in de sessie waar yaml met door-mij-verzonnen content werd gevuld (act-moods "tense, rising, climactic", kishotenketsu-prose-templates) is door de gebruiker gevangen en teruggedraaid. Yaml bevat nu alleen wat een tafel-bron of mechanische verdeling kan rechtvaardigen; setting-velden komen uit de voicing-call, niet uit yaml. Dit is een principe — yaml is consolidatie, geen creatie — en zit nu in de project-regels op systeem-prompt-niveau geborgen.
...
Concreet: `<reflect>`-block memory-window (8) versus `query_npc` memory-window (5) verenigd op één yaml-key `engine/npc.yaml::npc.reflection_observation_window`. Schema-required AI-output velden gelezen via direct subscript in `ai/brain.py`, `ai/narrator.py`, `ai/metadata.py`, `ai/architect.py`, `correction/orchestrator.py`, `correction/ops.py`, `correction/analysis.py`, `npc/processing.py`, `director.py`. Hardcoded correction-fallback dict in `correction/analysis.py` vervangen door yaml-keys onder `narrator_defaults`. `constraint_check_max_retries` (stale naam van de verwijderde constraint-validator) hernoemd naar `tool_loop_round_max_retries` over yaml plus dataclass plus handler. Dode `or ""`-fallbacks op altijd-string `snap.player_input` weg. `available_moves` raised nu op no-setting in plaats van een error-shape dict te retourneren. Dode `reflection_tone_fallback` yaml-key weg (geen Python-lezer). `find_npc` getypeerd als `str | None` zoals het altijd al feitelijk werkte.

Twee echte bugs gevonden tijdens de sweep. Eén: schema-veld `extra` in narrator_metadata werd door niemand gelezen, terwijl `_apply_description_updates` op zijn beurt een verzonnen veld `details` las dat nooit binnenkwam — twee dode kanten die elkaar niet kenden. Schema-veld plus code-blok beide weg. Twee: de director-prompt vroeg de AI om een `act_transition: bool` veld dat het director-schema niet heeft (`additionalProperties: False`) en geen Python leest — de act-overgangs-bepaling is engine-deterministisch in `_check_engine_act_transition`. Tokens en AI-attentie verspild op werk dat de engine al deed; latente fout bij strikter modelgedrag. Prompt-regel weg, twee mock-tests opgeschoond.

ARCHITECTURE.md kreeg een "Concrete callsite mapping"-paragraaf na "When tool-call vs when prompt-inject" — vier zinnen die per callsite-categorie noemen wat prompt-only is en welk veld het mixed Director-callsite met de tool-pad deelt. Save format unchanged. 1117 tests groen (was 1119, twee architect-tests verwijderd die `{}` aan schema-validated functies gaven — assumption-never-held), ruff clean, ruff format clean op 167 files, mypy clean op 100 source files, twintig project-rule scans clean.
...
Pass 1 (`e6fd7ed`): tweeëntwintig dode `Optional`-collection-defaults plus `or X`-fallbacks weg, plus bycatch op getouchte files. Strict signatures op alle prompt_blocks-helpers, `call_narrator`, `apply_narrator_metadata`, `register_extracted_npcs`, `_npc_block` en families (move_category required). `ai/schemas.py` opgesplitst in `_str` plus `_str_enum` plus `_str_with_desc` plus `_obj` plus `_obj_root` voor non-Optional helpers met acht callsites gemigreerd. `tools/registry.py` gesplitst in productie `register` plus `register_test_tool`. Magic number `creativity_seed n=3` naar yaml. Pass 2 (`0fbb79a`): AICallSpec-dataclass vervangt elf optionele kwargs op `AIProvider.create_message` en `create_with_retry` door één `spec`-parameter — de Optionals leven nu binnen één dataclass die de external-boundary AI-API shape modelleert, valt onder bestaande exception-2. Beide providers plus negen productie-callsites plus director-pipeline plus `run_tool_loop` gemigreerd. Test-MockProviders geconsolideerd in `tests/_mocks.py`. Pass 4 (`325264b`): drie sentinel-Optionals via splitsing opgelost — `resolve_fate_check` plus `resolve_fate_check_with_dice`, `process_deceased_npcs` plus `process_deceased_npcs_with_presence_check`, `_npc.secrets` via module-constant plus `tuple[str, ...]` default.

Elvira-sloop (`4915e24`): drift-check en GenreConstraints volledig weg. `tests/elvira/elvira_bot/drift_checks.py` verwijderd plus alle writers plus `drift_summary` field plus reader. `GenreConstraints` plus partial plus parser plus resolver plus property weg in `datasworn/settings.py`. `genre_constraints`-blok plus drie keys (`forbidden_terms`, `atmospheric_drift`, `atmospheric_drift_threshold`) weg uit alle drie shipped settings yamls. Twaalf dode rapportage-velden weg uit `SessionLog` (tien), `ChapterRecord` (één), `TurnRecord` (één) — allemaal write-only zonder code-reader buiten asdict-payload. md-audit (`2d7fd5e`): ARCHITECTURE.md drift-strategie-paragraaf herschreven naar "AI-surface reduction over post-hoc validation"; Settings YAML format plus Inheritance gestript van `genre_constraints`; CONTRIBUTING.md aantal scans van elf naar zeventien, Elvira-paragraaf herschreven, model_eval-paragraaf compleet weg.

Niet gedaan: ARCHITECTURE.md-tabel verwijst nog op meerdere plekken naar `engine.yaml` als afkorting voor het engine yaml-pakket terwijl de configuratie verspreid zit over `engine/<naam>.yaml` files. Doc-inconsistentie, geen sloop-residue, beslissing voor een aparte sessie. Test count: 1137 → 1138 (één test erbij voor `run_tool_loop_hits_max_rounds` die `pacing.max_tool_rounds` monkeypatcht via `dataclasses.replace`). Save format ongewijzigd.
...
GENRE PHYSICS en RESOLUTION PACING op narrator-niveau. Narrator-GLM-prompt was asymmetrisch met de universele variant — geen fact_budget-uitleg, geen voorbeelden met `fact_budget=N`-labels — terwijl `<target_npc fact_budget="N">` al in de input zat. NPC SPEECH CONTENT-block uitgebreid met fact_budget-uitleg en pattern-block-voorbeelden gelabeld met `fact_budget=1` en `fact_budget=2`. Voor GENRE PHYSICS drie maatregelen: TRANSFORMATION RULE toegevoegd ("when an inanimate subject would take an active verb, swap subject and agent so the player or environment performs the action"), positieve voorbeelden uit batch-data ("you press your palms against the iron rungs", "iron grinds against iron under your weight") en een nieuwe regex-laag `genre_physics_patterns` in `engine/validator.yaml` die metaal-en-architectuur-personificatie vangt voordat de LLM-validator hoeft te draaien. Acht regressie-tests met de exacte fragmenten uit de batch ("iron rungs bite", "the hole yawns", "the lock whines", "iron screams", "wind moans") plus negatieve tests voor correct gebruik en quoted-dialogue-uitzondering.

`(role, model_family)` resolution layer in z'n geheel verwijderd — een fallback-systeem voor modellen die niet draaien, ingebouwd in 2026.04.26.1 en sindsdien dood gewicht. De praktijk: één narrator-prompt voor één model (GLM), één validator-prompt voor één model (gpt_oss), drie lege `_overlays: {}` dicts en zeven dode helper-functies. Alles weg. `narrator_system_glm` hernoemd naar `narrator_system`, bare `narrator_system` (universele fallback) verwijderd. Idem `validator_system_gpt_oss` → `validator_system`. `*_universal`/`_overlays` keys in `engine/validator.yaml` en de drie `data/settings/*.yaml` files vervangen door bare keys (`agency_patterns`, `atmospheric_drift`). `compiled_patterns_for_family` plus `atmospheric_drift_for(family)` weg; consumers gebruiken nu `compiled_patterns(section, key)` en directe attribute-access. Drie family-helpers (`model_family_for_model`, `model_family_for_role`, `narrator_model_family`) plus `model_family`-veld op `AIConfig` plus `ai.model_family` config-block weg. `get_prompt(name, role="...")` vereenvoudigd naar `get_prompt(name)`; 25-plus callsites gestript. ARCHITECTURE.md sectie "Role, model_family resolution layer" verwijderd; CONTRIBUTING.md herschreven naar "switching to a different model means re-tuning the prompts and pattern lists in place, not adding parallel variants." 21 family-tests verwijderd uit `test_config_loader.py`, `test_prompt_loader.py`, en `test_pattern_family_resolution.py` (die laatste hele file weg).

Strict-rule violations gefixt op de eigen perimeter. Twee `or []`-fallbacks die in de RESULT INTEGRITY-implementatie waren geslopen weggehaald: `consequences=consequences or []` en `consequence_events=consequence_events or []` in `ValidationContext.build`. `Sequence[X] = ()`-defaults gebruikt voor empty-collection-defaults — strict-rules-conform en mypy-clean. `validate_and_retry`, `narrate_scene`, `apply_post_narration` in dezelfde stijl getrokken; `finalize_scene` in `scene_finalization.py` cascade-fix. 1225 tests groen (1245 voor cleanup, -21 family-tests, +1 phrasings-symmetrie). Ruff clean, ruff format clean op 181 files, mypy clean op 105 source files. Save format ongewijzigd.

---

## [2026.04.27.5] — 2026-04-27

Elvira batch-output structuur opgeschoond. Sessie-files werden tot dusver geschreven naar `tests/elvira/elvira_session_<YYYYMMDD_HHMM>.json` met minuut-precisie in de timestamp. Bij een snelle batch (negen sessies van vijf turns) draaiden meerdere runs binnen dezelfde minuut af, waarna `.write_text()` de eerdere file overschreef. Drie van de negen `explorer`-sessies gingen zo verloren in een recente batch-meting; de geaggregeerde data in `batch_report.json` overleefde, de turn-by-turn data niet.

Filename verrijkt met setting en style — `elvira_session_<setting>_<style>_<YYYYMMDD_HHMMSS>.json`. Drie × drie-batches produceren nu negen unieke namen op basis van inhoud, niet timing. Seconden-resolutie blijft als tweede laag voor toekomstige parallelle runs. Alle output (negen sessie-files plus `batch_report.json`) gaat naar een nieuwe `tests/elvira/runs/`-subdir; de batcher gooit die map weg en herbouwt 'm bij iedere start. Een directory om te zippen, een directory om weg te gooien.

In-scope rule fixes op de getouchte files. Drie `game_cfg.get("load_existing")` violations in `runner.py` en `ws_runner.py` naar direct subscript — `load_existing` staat in `elvira_config.yaml` als required key. Eén `setting_id = game_cfg.get("setting_id", "")` in `_setup_game` herschreven naar direct subscript plus expliciete `if setting_id == ""` guard voor de auto-mode random-roll opt-in (geen silent default; gedocumenteerd protocol-token). Vier `cfg.setdefault("game", {})["..."]` patronen in `elvira_batch.py` naar direct subscript op required yaml-secties. Twee dode `or ["starforged"]` fallbacks weggehaald — `list_packages() if s != "delve"` levert minstens drie settings, de fallback was onbereikbaar. Het `--output`-CLI-argument verwijderd: het overschreef alleen het rapport-pad maar de subdir werd toch gewist, wat een halfdood contract opleverde. `args.styles or ALL_STYLES` patronen vervangen door `args.styles if args.styles is not None else ALL_STYLES` zodat een lege explicit list niet stilletjes door de defaults wordt vervangen. Eén `self.config.get("game", {}).get("setting_id", "")` in `models.py::SessionLog.to_diagnostic_dict` naar direct subscript op dezelfde grond. Dood `_HERE` in `ws_runner.py` verwijderd; in `runner.py` ge-inlined als directe `RUNS_DIR`-bouwsteen.

Save format ongewijzigd. 1234 tests groen, ruff clean, ruff format clean, mypy clean op 105 source files. Vier files aangeraakt: `tests/elvira/elvira_batch.py`, `tests/elvira/elvira_bot/runner.py`, `tests/elvira/elvira_bot/ws_runner.py`, `tests/elvira/elvira_bot/models.py`.

---

## [2026.04.27.4] — 2026-04-27

Bundeling van bug-investigatie, truncation-diagnose, en architecturale tuning op RESOLUTION PACING en GENRE PHYSICS. Productie-bug in `web/serializers.py:211`: `opt.get("summary", "")` op truth-options. Voor classic (Datasworn-truths zonder `summary`-veld) leverde dit lege options aan de UI én lege `game.truths` in de narrator-prompt — narrator kreeg geen world-context op classic. Gefixt met format-tolerantie: `summary` als aanwezig, anders `description`. Doc-drift hersteld: ARCHITECTURE.md zegt `atmospheric_drift_overlays` is required (may be empty), loader was lax. Strict gemaakt; classic.yaml en starforged.yaml kregen expliciete `atmospheric_drift_overlays: {}` toegevoegd. Drie verdachten uit handover (chaos_thresholds high/low, disposition_shifts mist loyal, prompt_abbreviations mist none) bleken bewuste-design-keuzes na lezing van consumers — geen bugs.

Truncation-mysterie ontrafeld. De "appears truncated despite complete" warnings gingen niet over narrator-truncatie maar over GLM-4.7 op Cerebras die soms responses returnt met letterlijke `\u201c`/`\u201d`-escape-sequences (6 ASCII-tekens) in plaats van echte Unicode-karakters. Player zag `\u201c` in narratie in plaats van `"`. Decoder in `post_process_response` toegevoegd die alleen letterlijke `\uXXXX`-patronen vervangt — idempotent, veilig voor JSON-content. Vier tests in `test_provider_retry.py`. Bonus: hardcoded 500-char truncation in Elvira's `models.py` die narrations mid-woord knipte in session-files weggehaald — analyse werkt nu op volledige narrations.

Architecturale fact_budget-flow voor RESOLUTION PACING. Validator kreeg nooit te zien wat de narrator zag: de gate-berekening leverde een `fact_budget` per target-NPC (0/1/2 op basis van scenes_known, bond, en stance), maar alleen narrator gebruikte het. Validator beoordeelde zonder context en activeerde standaard "max ONE extra"-regel — terwijl narrator soms 2 mocht. Tegenstrijdigheid in validator-prompt zelf: regel zei "1 of 2 oké, 3+ violation", voorbeeld zei "2 = violation". `ValidationContext` uitgebreid met `target_npc_name` en `fact_budget`. Doorgegeven via `narrate_scene` → `validate_and_retry` → validator-prompt's `<context>`-tag. Beide validator-prompt-varianten (default + gpt_oss) herzien om `fact_budget` strikt te respecteren; voorbeelden gelabeld met `fact_budget=N` zodat regel en voorbeeld consistent zijn. Drie tests in `test_validator.py` voor de doorgifte plus update van `MockProvider` zodat `messages` ook in calls-log komt.

GENRE PHYSICS-narrator-prompt aanpassing. Concrete WRONG-voorbeelden in beide narrator-prompts ("the iron bites", "the hinges cry out", "the wood mocks", etc.) waren waarschijnlijk verbal priming — door deze tokens prominent te tonen activeerde de narrator juist het patroon. Vervangen door categorische beschrijving: "verbs of pain or injury, voice, breath, will or intent, awareness applied to inanimate subjects". RIGHT-voorbeelden behouden (positief-priming gewenst). Effect alleen meetbaar in volgende batch. 1234 tests groen, ruff clean, ruff format clean, mypy clean op 105 source files. Save format ongewijzigd.
...
Root cause van de KeyError: de v0 release breidde `move_categories.yaml` van 8 naar 92 moves uit en introduceerde `gather_information` als zesde categorie, maar twee andere yaml-tabellen die diezelfde categorie-set gebruiken werden niet gesymmetriseerd. `engine/stance_move_buckets.yaml mapping` miste `gather_information: gather_information`. `engine/position_resolver.yaml move_baselines` miste `gather_information: 0`. Beide aangevuld. Twee redundante hardcoded fallbacks rondom de mapping verwijderd: de `if move == "adventure/gather_information"` short-circuit in `_resolve_stance_category` en de `cat in (...) else "other"` collapse in `resolve_npc_stance`. De symmetrische mapping garandeert nu de juiste bucket; redundante administratie is geschrapt.

Brain-output sanitizer toegevoegd in `process_turn` direct na de Brain-call. Wanneer Brain een action_roll-move retourneert met `stat='none'` (Brain-fout, niet engine-fout), routeert de sanitizer de turn naar het dialog-pad in plaats van te crashen. De strict-raise in `_execute_roll` blijft staan als tweede beschermlaag voor wanneer iets de sanitizer zou omzeilen. Eén warning-log naar de console zodat Brain-fouten zichtbaar blijven zonder de sessie af te kappen.

Nieuwe testmodule `tests/test_yaml_symmetry.py` met tien tests die elke vorm van deze klasse bug afdwingt: elke categorie in `move_categories.yaml` moet voorkomen in `stance_move_buckets.mapping`, `position_resolver.move_baselines`, `memory_emotions.base` en `memory_result_text` (alle drie de results); elke bucket-waarde uit `stance_move_buckets.mapping` moet bestaan in elke `(disposition, bond_level)` cel van `stance_matrix`; elke move uit Datasworn plus engine_moves moet zonder crash door de hele stance-pipeline lopen. Vijf tests rond de Brain-output sanitizer in `tests/test_engine_memories.py`. Eén bestaande test in `test_stance.py` herschreven van "defaults to other" naar "raises KeyError" — strict-rules conform, errors propagate. 1218 tests groen, ruff clean, ruff format clean, mypy clean op 105 source files. Save format ongewijzigd.

---

## [2026.04.27.0] — 2026-04-27

Twee engine-bugs gefixt en validator/narrator-tuning na de v6 baseline-meting. Bug A: `generate_engine_memories` bouwde keys als `other_dialog`, `combat_dialog` voor het `roll=None`-pad bij `ask_the_oracle` en `dialog_only=True` op een non-dialog move. Vier asymmetrische "is dit een dialog-turn"-checks in `engine_memories.py` plus turn.py geconsolideerd in twee helpers `is_dialog_branch` (pre-roll, voor turn-loop) en `is_dialog_memory` (post-roll, voor memory- en scene-context). Daarnaast `move_categories.yaml` van 8 naar 92 moves uitgebreid — eerder viel 91% van alle moves door op `"other"`, wat ook de `gather_information` stance-resolutie stuk hield. Twee nieuwe categorie-keys (`gather_information_MISS/_WEAK_HIT/_STRONG_HIT`) toegevoegd in `memory_emotions.base` en `memory_result_text` zodat het mapping-bereik symmetrisch is.

Bug B: `_execute_roll` raised nu een diagnostische `ValueError` wanneer Brain `stat="none"` returnt op een action_roll-move, in plaats van `Unknown stat: 'none'` diep in `get_stat`. Plus een turn-guard: `process_turn` weigert te draaien op `game.game_over=True` met een leesbare RuntimeError — tweede beschermlaag voor wanneer een caller (web-handler, Elvira-runner) de game-over check overslaat.

Validator-tuning op drie meet-bewezen false positives. Het body-press agency-pattern (`(presses?|settles?|weighs?|hangs?|bears? down) ... your (chest|shoulders|ears|...)`) verwijderd uit `agency_patterns_universal` — het ving sensorische gewaarwordingen die de validator-prompt zelf als SAFE markeert. Het silver-lining-pattern `but you ... (find|discover|...)` voorzien van negative lookahead op `nothing|no|none|without` zodat `but you find nothing` niet langer als positief gevolg telt. Sundered Isles drift-lijst gesnoeid: `weep`, `weeping`, `shimmers`, `shimmering` weg — legitiem in gothic-marine setting; `_overlays: {}` toegevoegd zoals architectuur voorschrijft. Andere drift-woorden behouden tot Elvira-meting bewijst dat ze ook problemen geven.

Equivalence-clausule toegevoegd aan beide `validator_system` varianten in RESULT INTEGRITY: "the narrator may describe a consequence in different words; only the event must be present." Eén RIGHT-voorbeeld als anker. Narrator-prompt (beide varianten) krijgt expliciete player-as-agent-regel voor WEAK_HIT en STRONG_HIT — speler moet grammaticaal subject zijn van de successful action, geen WRONG-voorbeeld om patroon-activatie te vermijden.

Twintig nieuwe regressietests in `tests/test_engine_memories.py` voor de helpers, de KeyError-paden, de turn-guard, de stat=none detectie, de move-categories volledigheid en memory yaml symmetrie. 1205 tests groen, ruff clean, ruff format clean, mypy clean op 105 source files. Save format ongewijzigd. Niet aangeraakt deze sessie: RESOLUTION PACING (47 instances, ~30%) en GENRE PHYSICS anthropomorfismen (25 instances, ~16%) — vragen architecturale aanpak (NPC-whitelist via gate-laag respectievelijk WRONG-voorbeelden uit prompt verwijderen) en wachten op een volgende meting.
...
`prompt_blocks.py` volledig config-driven. Negen blok-functies herschreven, alle hardcoded XML-wrappers, label-strings en format-templates verhuisd naar 25 nieuwe keys in `prompts/blocks.yaml`. Het narrative-direction-token-vocabulair (intensity-vier, tempo, perspective, player-twee, position-drie) plus de wrappers voor content_boundaries, character_state, tone_authority, vocabulary, world_truths, story_arc, revelation_ready, recent_events, campaign_history zijn nu yaml. `get_narrator_system` past `vocabulary_block` en `world_truths_block` als format-vars in plaats van append-end — daardoor kan de yaml-template hun positie sturen. In de GLM-variant staan ze in de primacy-zone direct na de hard MUST-constraints, in de universal-variant achteraan zoals het oude gedrag was.

Elvira-bot dezelfde behandeling. Vijf style-personas (`explorer`, `aggressor`, `dialogist`, `chaosagent`, `balanced`) herschreven in PURPOSE / ACTION RULES / OUTPUT structuur met harde MUST-taal — "NEVER" en "Don't" vervangen door "MUST NOT", soft language elimineerd, output-contract per style expliciet. `burn_decision` opgesplitst in een paired prompt: `burn_decision_system` ("answer with exactly one word") plus de user-template. `tests/elvira/elvira_bot/ai_helpers.py` van hardcoded naar yaml-driven: tien turn-directive teksten, een rotatie-list, een MANDATORY-action-block-template, een prev-action-block-template, het complete `bot_turn_context` template, plus de "(unknown)" en "(none)" labels en de NPC/clock/track per-line formats. Nieuwe `_p(key, **kwargs)` helper voor type-safe yaml-prompt-resolution; `_resolve_turn_directive(turn)` leest de rotatie-list modulo turn-index. ARCHITECTURE.md drift gefixt — drie obsolete Qwen-mentions vervangen, file-map-omschrijving van `prompt_blocks.py` uitgebreid. Geen save-format wijzigingen. 1187 tests groen, ruff clean, ruff format clean, mypy clean op 105 source files.
...
Twee grab-bag testfiles ontmanteld: `tests/test_engine.py` en `tests/test_coverage.py` opgeheven, inhoud verspreid naar gerichte files (`test_config_loader.py`, `test_validator.py`, `test_architect_validator.py`, `test_setup_common.py`, `test_provider_retry.py`) plus aangevuld op bestaande files (`test_resolvers.py`, `test_parser.py`, `test_npc.py`, `test_director.py`). Shared mock-infrastructuur in nieuwe `tests/_mocks.py` (MockProvider, MockResponse, make_test_game). Verplaatsingen: vijf progress-track-tests van `test_creation.py` naar `test_track_lifecycle.py`, vier CombatPosition-tests van `test_move_outcome.py` naar `test_models.py`, twee track-finder-tests van `test_move_outcome.py` naar `test_track_lifecycle.py`. Imports opgeschoond, dode `if __name__ == "__main__"` block uit `test_npc.py` verwijderd.

Coverage van 81% naar 88% via zes nieuwe testfiles plus aanvullingen: `test_prompt_boundary.py` (10 tests, prompt_boundary.py 23→98%), `test_world.py` (29 tests, mechanics/world.py 58→99%), `test_story_state.py` (21 tests, story_state.py 51→98%), `test_json_utils.py` (10 tests, ai/json_utils.py 17→92%), `test_engine_loader.py` (7 tests, engine_loader.py 52→93%), `test_architect.py` (26 tests, ai/architect.py 28→97%), `test_chapters.py` (22 tests, game/chapters.py 37→74%), `test_game_start.py` (10 tests, game/game_start.py 54→94%), `test_web_handlers.py` (38 tests, web/handlers.py 31→50%). Plus 4 succession-tests aan `test_succession.py` (game/succession.py 59→98%) en 3 run_tool_loop-tests aan `test_tools.py`.

Performance-verbetering in `test_project_rules.py`: shared `_FILE_CACHE` plus `_load(path)` helper hergebruikt parse + parents-map + lines tussen de elf scans. Eerder parsed elke scan elke file opnieuw. De elf losse `test_*` functies zijn geconsolideerd tot één `test_project_rules` met elf `_check_*` helpers — bij failure groepeert de assertion alle violations per categorie. Full pytest-run van 10,9s naar 8,4s (-23%). Test count 1019 → 1187 (+168 toegevoegd, 11 rule-tests samengevoegd tot 1, geen test verwijderd uit dekking). Save format ongewijzigd. Ruff clean, ruff format clean, mypy clean op 105 source files, project-rule scan clean.
