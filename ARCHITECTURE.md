...
AC's role is bounded to plot-skeleton at chapter boundaries plus keyed-scene-spawning. Step 7a wired the blueprint path: `assemble_blueprint_seed_from_ac` rolls turning-points and produces a `BlueprintSeed` (frozen dataclass), which `call_blueprint_voicing` converts to the setting-specific `StoryBlueprint` via one structured AI call. The voicing call only translates engine-rolled structure into setting prose; it does not pick plot beats. Kishōtenketsu blueprints follow the same shape but skip turning-point rolling — `assemble_blueprint_seed_kishotenketsu` emits four phase-acts read from yaml, and the voicing call fills the setting fields. Step 7c adds keyed-scene-spawning when AC turning-points map onto registered engine triggers. Mid-chapter scheduling of AC turning-points is deliberately not done — pacing within a chapter runs on the six existing systems (Mythic random events, pacing-hint, clocks, threats, scene-structure, Director NPC-reflections), not on a seventh AC-driven layer (see "AC scope boundary" below).
