...
| Adventure Crafter primitives (themes, plot points, meta dispatch, turning points, supporting tables, character crafting) | `mechanics/adventure_crafter.py` → `assign_themes`, `lookup_plot_point`, `lookup_meta_plot_point`, `dispatch_meta`, `roll_turning_point`, `lookup_theme_priority`, `lookup_characters_template`, `lookup_plotlines_template`, `roll_character_traits`, `lookup_character_special_trait`, `lookup_character_identity`, `lookup_character_descriptor`; `engine/adventure_crafter.yaml` (themes, theme_die_table, special_ranges); `data/adventure_crafter.json` (lookup tables, turning_point_rules, plot_point_theme_priority, characters_list_template, plotlines_list_template, character_special_trait, character_identity, character_descriptors) |
...
**Keyed scenes are the engine's pre-scheduled beat channel.** Triggers are registered in `engine/keyed_scenes.yaml`; each registered name maps to an evaluator in `mechanics/keyed_scenes.py::_EVALUATORS`. `KeyedScene.__post_init__` validates `trigger_type` against the registered set, and for pattern-supporting triggers also validates `trigger_value` against the per-trigger grammar. The matched scene is consumed (one-shot) by `check_scene`. Three sources spawn keyed scenes: AC at chapter-start (campaign-skeleton beats from turning-point plot-points), random events at fire-time (Mythic-event focus mapping concrete-target triggers), and clock creation at game-start plus chapter-start (per clock-type fractions emit tussenstop-keyed-scenes via `apply_world_setup`).

**Pattern grammar across pattern-supporting triggers.** Three trigger-types accept patterns: `bond_threshold` (`<npc_id>:N`, `any:N`, `disposition_<value>:N`), `threat_menace_phase` (`<threat_name>:N`, `any:N`), and `clock_fills` (`<clock_name>:N`, `any:N`). `engine/keyed_scenes.yaml::pattern_grammars` lists per-trigger valid prefixes plus a matcher_strategy (`highest_bond`, `highest_menace`, `highest_filled`). On fire-moment `evaluate_keyed_scenes` resolves a pattern-trigger to a concrete entity according to the strategy, sets `KeyedScene.bound_entity_id`, and excludes already-bound entities from siblings' resolution. `chaos_extreme` and `scene_count` are entity-less triggers without patterns. AC's `keyed_scene_mapping` carries 33 plot-point-to-trigger mappings spread across all five trigger-types with varied thresholds and priorities; dedup-A skips spawning the same plot-point twice within a chapter, dedup-B caps total AC keyed-scenes per chapter at `max_keyed_scenes_per_chapter`. Random-event-spawner builds concrete triggers from `event.target_id` (bond) or `event.target` (threat name); clock-spawner reads per-clock-type fractions from `engine/clock_keyed_scenes.yaml` and templates the clock name into the narrative_hint.

**Adventure Crafter primitives.** AC (Pigeon, Word Mill Games) provides plot-level structure that complements Mythic GME 2e's scene-level chaos. The primitives live in `mechanics/adventure_crafter.py`: theme assignment over five canonical themes (action, tension, mystery, social, personal) via a d10 table, plot-point lookup keyed by `(theme, roll)` because the entries in `data/adventure_crafter.json` carry sparse theme coverage (most entries declare ranges on only a subset of themes), special-range flagging on `Conclusion (1-8)`, `None (9-24)`, and `Meta (96-100)`, and a meta-handler dispatch table covering the seven meta-plot-point types. Engine-level configuration lives in `engine/adventure_crafter.yaml` (themes, theme_slots, theme_die_table, special_ranges); the lookup data lives in `data/adventure_crafter.json`. The yaml `theme_die_table` is cross-validated against the JSON `random_themes` block on first load — mismatches raise at parse time so the engine never silently diverges from the AC data file. Character-crafting tables (special trait, identity, descriptors) ship as pure helpers `roll_character_traits` plus three per-table lookup helpers; the d100 flag-ranges (1-33 dual identity, 1-21 dual descriptors) trigger two restricted-range rerolls each on 34..100 / 22..100 so the resulting `CharacterTraits` carries 1-or-2 identities and 1-or-2 descriptors with list-length as the discriminator. `CharacterTraits` has no consumer in src/ — step 11 (NPC tiers) wires it into NPC-data generation; until then it is a pure function over loaded AC data and lives under an explicit orphan-symbol carve-out in `tests/test_project_rules.py`.
...
AC's role is bounded to plot-skeleton at chapter boundaries plus keyed-scene-spawning. Step 7a wired the blueprint path: `assemble_blueprint_seed_from_ac` rolls turning-points and produces a `BlueprintSeed` (frozen dataclass), which `call_blueprint_voicing` converts to the setting-specific `StoryBlueprint` via one structured AI call. The voicing call only translates engine-rolled structure into setting prose; it does not pick plot beats. Kishōtenketsu blueprints follow the same shape but skip turning-point rolling — `assemble_blueprint_seed_kishotenketsu` emits four phase-acts read from yaml, and the voicing call fills the setting fields. Step 7c wired keyed-scene-spawning: `assemble_blueprint_seed_from_ac` now calls `spawn_keyed_scenes_for_turning_point` after each rolled turning-point, which reads `keyed_scene_mapping` and constructs `KeyedScene` instances on `narrative.keyed_scenes`. AC is now fully wired into engine-resolved fiction — blueprint at game-start and chapter-start, plus pre-scheduled beats via the keyed-scene-spawner. Mid-chapter scheduling of AC turning-points is deliberately not done — pacing within a chapter runs on the six existing systems (Mythic random events, pacing-hint, clocks, threats, scene-structure, Director NPC-reflections), not on a seventh AC-driven layer (see "AC scope boundary" below).
