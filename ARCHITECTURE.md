...
│   ├── ai/
│   │   ├── api_client.py    # get_provider(): routes each call to its role's provider; check_configured_models(): startup check; provider_named(): one configured provider's adapter (Elvira's judge); lazy SDK import
