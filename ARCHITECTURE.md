...
Six places where Straightjacket departs from the design document's architectural recommendations or settles one of its open questions. Each was an explicit decision, taken on empirical grounds or as a deliberate scope boundary, not a discard from the document.
...
**Degradation instead of a pause on AI failure.** The design document says that when an AI call fails or returns unusable output, the engine changes no game state, the game pauses, and the player gets a clean message. Straightjacket plays on instead, through the AI-call exception carve-out described under Key Design Decisions: the roll and its consequences are applied before narration, so a narrator failure leaves them in place with an empty narration; a failed Brain call becomes a dialog turn; a failed revelation check counts as confirmed; failed extraction and Director calls are skipped. The reason is practical, since transient provider faults are common and a crash per fault ends the session. The cost is that failures are invisible to the player and were invisible to the developer until Elvira began reporting every engine warning (2026.09.24.42). A clean player-facing message on AI failure is not implemented.
